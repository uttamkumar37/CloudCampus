...
| Infrastructure | Docker, local staging compose, Nginx, env contract, monitoring alert rules, backup/load-test placeholders | Backend/frontend Dockerfiles, local PostgreSQL-backed staging compose, frontend Nginx proxy config, staging env example, smoke script, Prometheus alert-rule scaffold, and local PostgreSQL backup/restore drill scripts exist | Repo-level staging scaffold exists, builds locally, runs backend against PostgreSQL 16, and has a local backup/restore drill that restored 45 public tables. No hosted staging environment, managed production secret store, image registry publish, TLS/domain, scheduled managed backups, production RTO/RPO policy, or deployed Prometheus/Grafana/Alertmanager stack exists yet. | Required before pilot: choose hosting target, wire managed PostgreSQL/secrets/TLS/registry/scheduled backups/alerts, and prove managed restore. |
| CI/CD | `.github/workflows/ci.yml`, `.github/workflows/security.yml`, `.github/dependabot.yml`, plus `.github/workflows/.gitkeep` | GitHub Actions workflow present for backend, frontend, mobile, ops scaffold, security audit, and nightly/manual security scans | Current CI runs backend Maven tests, frontend `npm ci`/tests/lint/typecheck/build, mobile `npm ci`/lint/typecheck/tests, ops scaffold validation plus staging compose config, and high/critical npm production audit gates. Security workflow adds PR dependency review, CodeQL, and Trivy filesystem SARIF scanning on PR/nightly/manual. Dependabot is configured for backend Maven, frontend npm, mobile npm, and GitHub Actions. No image publish, OpenAPI publish, scheduled DR workflow, or hosted staging deploy exists yet. | Required before pilot: add image publish, real staging deploy, OpenAPI publish, scheduled backup/restore workflows, and tune scanner false-positive policy as dependencies mature. |
...
| CI/CD | GitHub Actions CI, Docker publish, security nightly, OpenAPI publish, DR drill, deploy placeholder | PARTIAL in historical audit; current scaffold now has `ci.yml` for backend/frontend/mobile tests/builds/lint gates plus ops scaffold validation, compose config validation, and high/critical npm audit gates. `security.yml` adds dependency review, CodeQL, and Trivy filesystem scanning; Dependabot is configured. Deploy workflow was previously placeholder and is not rebuilt. | Required before pilot: add image/build publish and real staging deploy. |
| Deployment | Docker Compose, backend/frontend Dockerfiles, local Nginx TLS proxy, starter Kubernetes YAML | PARTIAL. Current scaffold now has backend/frontend Dockerfiles, local staging compose, frontend Nginx SPA/API proxy config, staging env contract, and smoke script. No hosted production IaC. | Required before pilot: managed DB/object storage/secrets/backups/HTTPS/alerts. |
...
- Backend health became `healthy` against PostgreSQL 16 after Flyway applied V1-V26.
...
- `mvn -f backend/pom.xml test` passed with 132 tests, 0 failures, 0 errors, 0 skipped.
...
- `cd backend && mvn -q test` passed at AI-001 completion; the current full backend suite now reports 132 tests after AI-002 was added.
...
Recommended next implementation task: continue first-version backend/product readiness rather than expanding AI; live model/vector/provider work belongs to a later AI version. `SEC-002` remains the standing security gate to strengthen alongside each rebuilt school-scoped module.
...
- `cd backend && mvn -q test` passed.
- Surefire reports 132 tests, 0 failures, 0 errors, 0 skipped.
...
| Backup/Restore Test | `docker compose -f infra/docker/local/compose.staging.yml up -d --build postgres backend`; `sh scripts/ops/backup-local-postgres.sh`; `sh scripts/ops/restore-local-postgres-drill.sh`; `mvn -f backend/pom.xml test`; `sh scripts/ci/validate-ops.sh`; `docker compose -f infra/docker/local/compose.staging.yml down` | PASS/PARTIAL | 2026-05-27 | OPS-004 verified local backup/restore only: backend ran healthy against PostgreSQL 16 with Flyway V1-V26, backup script created ignored custom-format dump `infra/backup/local/cloudcampus-cloudcampus-20260527T064121Z.dump` plus manifest, restore drill restored into `cloudcampus_restore_drill`, verified 45 public tables, and dropped the drill DB. `mvn -f backend/pom.xml test` passed with 132 tests. Remaining production gaps: managed cloud backups, encrypted offsite retention, scheduled restore drill, RTO/RPO, and production runbook. |
