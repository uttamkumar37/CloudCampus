...
| CI/CD | `.github/workflows/ci.yml` plus `.github/workflows/.gitkeep` | GitHub Actions workflow present for backend, frontend, mobile, and ops scaffold gates | Current CI runs backend Maven tests, frontend `npm ci`/tests/lint/typecheck/build, mobile `npm ci`/lint/typecheck/tests, and ops scaffold validation plus staging compose config on push, pull request, and manual dispatch. No image publish, security nightly, OpenAPI publish, DR drill, or dependency/security scan job exists yet. | Required before pilot: add security/dependency scans, image publish, staging deploy, OpenAPI publish, and backup/restore workflows. |
...
| CI/CD | GitHub Actions CI, Docker publish, security nightly, OpenAPI publish, DR drill, deploy placeholder | PARTIAL in historical audit; current scaffold now has `ci.yml` for backend/frontend/mobile tests/builds/lint gates plus ops scaffold validation and compose config validation. Deploy workflow was previously placeholder and is not rebuilt. | Required before pilot: add security scans, image/build publish, and real staging deploy. |
...
Status decision: `OPS-002` is VERIFIED for the current repository-level staging/ops scaffold. Remaining gaps before pilot are a real hosted staging target, managed secret store, production SMTP secret injection, image registry publish, TLS/domain setup, managed PostgreSQL/object storage, Prometheus/Grafana/Alertmanager deployment, backup/restore proof, dependency/security scans, and DR workflow.
