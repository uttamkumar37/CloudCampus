...
Validation date: 2026-05-28, Asia/Kolkata. Historical code audit was performed on branch `project-redesign` at commit `a3c4328`; current scaffold recheck was performed after the redesign cleanup.

Current redesign state: after cleanup, the legacy application source is deleted from the working tree and a clean CloudCampus monorepo scaffold has been created. STRUCT-002 added minimal backend, frontend, and mobile build manifests plus tested baseline app shells. ONB-001, AUTH-001, AUTH-002, AUTH-003, AUTH-004, AUTH-005/SEC-004, ONB-002, ONB-003, ONB-004, SEC-001, SEC-002, SEC-003, SEC-006, PAR-001, ACA-001, STU-001, STAFF-001, ACA-002, AUD-001, EVT-001, BULK-001, STU-002, STU-003, FEE-001, FEE-002, CI-001, OPS-001, OPS-003, OPS-004, DEPLOY-001, DEPLOY-002, DEPLOY-003, DEPLOY-004, DEPLOY-005, DEPLOY-006, DEPLOY-007, DEPLOY-008, DEPLOY-009, FRONT-AUTH-001, FRONT-HOME-001, FRONT-UX-001, FRONT-UX-002, ATT-001, HOME-001, EXAM-001, NOTE-001, NOTIF-001, REP-001, MUL-001, MUL-002, MUL-003, MUL-004, MUL-005, MUL-006, MUL-007, SUB-001, AI-001, and AI-002 first-version scope are verified in the current scaffold. SEC-002 and SEC-006 now have current-scaffold route-level school object isolation across rebuilt school-scoped modules: parent-child access, staff/teacher/finance provisioning, academic lifecycle, academic assignment, student import/job/login flows, bulk jobs, fees/payments, attendance, homework, exams/results, notices, reports, timetable, documents, website content, and School Admin AI knowledge documents. Teacher routes prove assignment checks where teacher-facing APIs exist; parent/student routes prove linked-child or own-profile checks where those APIs exist; and `SchoolScopedControllerGuardCoverageTest` fails future unguarded school-scoped controllers. MUL-002 adds the first Tenant Admin multi-school creation path with server-derived tenant scope, durable school-limit guardrails, and audit coverage. MUL-003 verifies school switching across backend, web, and mobile shell role coverage. MUL-004 adds Tenant Admin combined tenant report summaries with school-safe drilldown. MUL-005 adds Tenant Admin School Admin invitation and `user_school_access` grant workflow for tenant schools. MUL-006 adds Tenant Admin school list/name-edit/deactivate and School Admin list/resend/revoke workflow with audit coverage and minimal web scaffolding. MUL-007 adds Tenant Admin tenant settings and subscription usage APIs, audit coverage, and minimal web scaffolding. NOTIF-001 adds SMTP-capable invitation email delivery with a safe local logging mode, durable delivery rows, and token-safe outbox metadata. SUB-001 adds Super Admin subscription plan catalog, tenant subscription assignment, invoice issuance scaffold, and plan-backed school-limit enforcement. AI-001 adds tenant-level AI entitlement, monthly unit budget, feature enablement, human-approval flag, and raw-prompt-safe usage audit foundation. AI-002 adds first-version scoped school knowledge documents and retrieval for `SCHOOL_POLICY_QA`, deriving tenant/school/role from authenticated context, active school grants, teacher grants, parent-child links, or student profile ownership. FEE-002 adds a `FINANCE_STAFF` login role, MFA-protected finance staff invitation/provisioning, finance-only fee demand/payment routes, and minimal web/mobile finance role surfaces. FRONT-HOME-001 adds a premium public SaaS homepage with dashboard/mobile previews and slide-over universal login access. FRONT-UX-001 adds a premium authenticated app shell with Lucide navigation, command palette, notification/profile popovers, Recharts dashboard analytics, quick actions, and mobile record cards. FRONT-UX-002 adds role-aware dashboard headers with live date/time, active user/session context, active school display, profile metadata, and role-specific information panels. OPS-001 adds ESLint static-analysis policy for frontend/mobile and gates both in CI. OPS-003 adds high/critical npm audit gates, Dependabot coverage, and a nightly/manual security workflow with dependency review, CodeQL, and Trivy filesystem scanning. OPS-004 adds local PostgreSQL staging backup and restore drill scripts, verifies Flyway migrations against PostgreSQL 16, and proves a custom-format dump can be restored into a throwaway database. DEPLOY-001 through DEPLOY-009 add root compose files, safe env templates, Nginx reverse proxy config, deployment/rollback/health runbooks, manual image build/deploy workflow, optional split-hosting API base support, local Docker smoke validation, Ubuntu EC2 bootstrap, and EC2 compose deployment helper. The implementation audit below is retained as historical evidence from the pre-cleanup codebase; new implementation work must rebuild inside the scaffolded structure and update this document as reality changes.

Current task update: PORTAL-PROD-STATE-001 removes pending-style portal states from visible role navigation. Backend now has SUPER_ADMIN-only platform control APIs for tenant list/detail/status/settings/tabs, schools directory/detail, revenue summary/invoices/trends, AI usage summary/tenant usage, reports summary/export list/request, audit-log exploration, platform health, notification delivery summary/list/detail, and safe platform settings. Frontend now renders visible portal sidebar items only when they call real backend APIs, hides missing/future-only modules from local and production navigation, removes `Partial API`/`Missing API` badges, and gives every role dashboard a live summary-backed workspace instead of a coming-soon fallback. Remaining work such as inline AI entitlement editing, subscription tenant-assignment/edit drawers, durable platform-level report export workers, durable platform settings persistence, real payment collection/reconciliation, and richer CRUD polish is tracked as post-MVP product hardening rather than visible pending navigation.

Current task update: UX-PORTAL-COPY-001 removes technical/debug implementation language from the normal CloudCampus portal UI. Frontend session and role context cards now use human-readable labels such as Account Session, Your role, Organization, Assigned schools, Current school, Available modules, Available tools, and Ready. Super Admin now sees a business/platform dashboard with platform access, organizations, schools, users, health, security, onboarding, subscription activity, notification delivery, quick actions, and audit alerts instead of tenant UUIDs, zero allowed-school counts, endpoint names, or security-engineering explanations. Technical details are hidden behind local-development-only Developer details sections and remain unavailable in staging/production/test mode. Super Admin sidebar navigation is grouped into Platform, Business, Operations, and Settings.

Current task update: API-UI-INTEGRATION-AUDIT-001 adds a full UI/API matrix and backend API inventory, then closes the highest-impact visible gaps found in the pass. Backend now includes paginated School Admin parent, teacher, and staff directory APIs; School Admin settings read/update APIs with authenticated audit; and Finance Staff receipt plus report summary/collection APIs. Frontend now wires School Admin Parents/Teachers/Staff/Settings and Finance Staff Receipts/Reports to those real endpoints through the shared HTTP client. Validation evidence added: targeted backend tests for staff, parent, finance, school settings, and audit coverage pass; frontend `App.test.tsx` now verifies the new route calls and passes.

Current task update: PROD-READINESS-AUDIT-001 creates `docs/audit/PRODUCTION_READINESS_REPORT.md` and gives the strict readiness verdict. CloudCampus is `STAGING_READY` for a controlled staging deployment attempt from the repository, local development and internal demos are ready, but pilot, paid production, and enterprise production are not ready until live staging, hosted backup/restore, monitoring, SMTP provider delivery, object storage, payment reconciliation, ops validation, and UX hardening are complete. Latest validation passes backend tests, frontend tests/lint/typecheck/build, mobile lint/typecheck/tests, security-audit high/critical gates, and local/staging/prod compose rendering. `scripts/ci/validate-ops.sh` currently fails because it flags the local example Super Admin password in `.env.example` and local compose fallback; OPS-VALID-001 is the next P0 readiness task.

Current task update: OPS-VALID-001 separates local development credential policy from staging/production deployment policy. `.env.example` and `docker-compose.local.yml` now clearly mark local-only bootstrap placeholders, while `scripts/ci/validate-ops.sh` validates staging/production deploy assets for unsafe local defaults, H2 URLs, bootstrap enablement, blank/unsafe secret posture, explicit HTTPS CORS origins, and production SMTP/log-mode rules. `infra/env/staging.example.env` now uses staging placeholders instead of local DB passwords. Validation evidence: `sh scripts/ci/validate-ops.sh` passes, and local/staging/production compose configs render with their example env files. CloudCampus is now cleanly ready for an EC2 staging deployment attempt; live staging is still not verified.

Current task update: STAGE-001 adds `docs/deployment/STAGING_RUNBOOK.md` and `docs/audit/STAGING_SMOKE_TEST_REPORT.md`. The runbook covers EC2 requirements, security groups, Docker/bootstrap, staging env setup, JWT secret generation, PostgreSQL, same-origin frontend/backend/CORS setup, mail mode, Nginx/TLS, deployment commands, health checks, log capture, functional smoke steps, and rollback. Local repo validation passes: backend tests, frontend tests/lint/typecheck/build, ops validation, and staging compose config. Live EC2 HTTP staging smoke was verified on `http://65.1.147.247`: backend health/readiness returned `UP`, frontend loaded through Nginx, Super Admin login/MFA worked, Super Admin onboarding created a tenant/first school/School Admin invitation/access grant, School Admin invitation acceptance worked, School Admin MFA login worked, academic class/section setup worked, student import created one student, and logout worked. The EC2 instance was then terminated to avoid ongoing AWS cost; AWS resource review showed no running instances, Elastic IPs, EBS volumes, snapshots, or load balancers. This is `LIVE_STAGING_HTTP_VERIFIED_THEN_TERMINATED`, not pilot-ready: no stable staging host, domain/TLS, SMTP delivery, rotated shared-staging secrets, disabled shared bootstrap, backup/restore drill, or monitoring/alerts are verified yet. The smoke also found a P0 UX issue at the time: School Admin student import exposed raw class/section UUIDs instead of school-aware dropdown selectors. That issue is now addressed by `UX-STU-IMPORT-001` and should be re-smoked in the next live staging environment.

Current task update: UX-STU-IMPORT-001 replaces the technical School Admin student import scaffold with a school-aware import workflow. Frontend now loads academic years, classes, and sections from the backend through the shared API client, auto-selects active/default scope, hides internal IDs, supports manual multi-row entry and simple CSV upload, validates rows before import or queueing, shows row-level validation summaries plus loading/empty/error/success states, and changes student login invitation from a raw Student ID field to a student selector. Validation evidence: `cd frontend && npm test -- --run`, `cd frontend && npm run lint`, `cd frontend && npm run typecheck`, and `cd frontend && npm run build` pass. No backend changes were required.

Current task update: UX-ACA-FORM-001 replaces raw-ID School Admin academic setup with guided selector workflows. `AcademicSetupPage` now loads academic years, classes, and sections from real backend APIs, shows existing records, handles empty/loading/error/success states, and creates classes/sections through academic-year and class selectors instead of visible UUID fields. `AcademicAssignmentsPage` now loads academic years, classes, subjects, class-subject assignments, and teacher directory rows, then assigns subjects and teachers through normal selectors plus teacher search instead of raw class/subject/teacher/class-subject IDs. Validation evidence: `cd frontend && npm test -- --run`, `cd frontend && npm run lint`, `cd frontend && npm run typecheck`, and `cd frontend && npm run build` pass. No backend changes were required.
...
|---|---|---|---|---|
| Backend | `backend/src/main/java/com/cloudcampus/...` domain folders, onboarding/invitation/access/audit/entities and APIs, login/session APIs, parent-child linking and leave request APIs, staff/teacher provisioning API, academic lifecycle APIs, academic assignment APIs, student import validation/job APIs, student login invitation/self-profile APIs, bulk job APIs, fee demand/payment/receipt APIs, attendance APIs, homework APIs, exam/result APIs, notice APIs, timetable APIs, document APIs, website content APIs, notification delivery foundation, report export APIs, Tenant Admin school creation/report/settings APIs, Super Admin subscription plan/assignment APIs, AI entitlement/usage-audit/scoped school knowledge retrieval APIs, transactional outbox foundation, readiness controller, tenant context spoofing filter, tests | Present: `backend/pom.xml`, Flyway migrations, H2 dev/test datasource | Runnable Spring Boot 3.5.14 baseline with tenant + first school + School Admin invitation + access grant + onboarding audit log + signed stateless access token login/current-user/school activation APIs, refresh-token rotation, logout revocation, password reset/change lifecycle, privileged-user MFA challenge verification, in-process login rate limiting, SUPER_ADMIN-only tenant onboarding authorization, parent-child linking with parent invitation/access checks, parent leave request create/list/decision APIs, School Admin staff/teacher portal-login provisioning, School Admin academic year/class/section lifecycle, class-subject/teacher assignment foundation, synchronous School Admin student import validation/import foundation, queued student-import job foundation, optional School Admin student-login invitation/provisioning with student self-profile, fee demand/payment/receipt lifecycle foundation, attendance session/record foundation for School Admin and teacher flows, homework publish/read/submission foundation for School Admin, Teacher, Parent, and Student flows, exam creation/marks/publish/result visibility foundation for School Admin, Teacher, Parent, and Student flows, school-scoped notice create/list/read/publish plus Teacher/Parent/Student visibility foundation, school-scoped timetable entry foundation, school-scoped document metadata foundation, school-scoped website page draft/publish foundation, SMTP-capable invitation email delivery with durable delivery rows and safe local logging, durable report export job/file foundation, Tenant Admin additional-school creation guarded by `tenant_school_limits`, Tenant Admin combined report summary and school-safe report drilldown, Tenant Admin tenant settings plus subscription usage summary, Super Admin subscription plan catalog and tenant assignment with invoice scaffold, Super Admin AI entitlement management plus authenticated tenant AI entitlement/usage-audit APIs, School Admin AI knowledge document create/list APIs, authenticated scoped AI knowledge search API, V10 transactional outbox events, V11 durable generic bulk jobs with School Admin school-access guards, V12 student import jobs, V13 student login link, V14 fee payment receipts, V15 attendance tables, V16 homework tables, V17 exam/result tables, V18 notices, V19 report export jobs/files, V20 tenant school limits, V21 tenant settings, V22 notification deliveries, V23 subscription plan/catalog tables, V24 parent leave request tables, V25 AI entitlement/audit tables, V26 AI knowledge document/retrieval audit tables, and V27 timetable/document/website foundation tables. Client-supplied tenant/school context headers are blocked on `/v1/**`. No production out-of-band MFA delivery, device-session management, outbox dispatcher, scheduled bulk/report worker runtime, object-storage export/document delivery, payment gateway/webhook integration, parent/student attendance portal routes, bounce/retry notification worker, provider template management, payment collection/reconciliation for invoices, live AI model gateway/vector RAG retrieval, or full frontend/mobile UX for newer backend modules yet. | Required now: implement only the next approved task. |
| Frontend | `frontend/src/app`, `frontend/src/shared`, role-based feature folders, auth state provider, protected panels, school selector, Super Admin onboarding screen, invitation acceptance screen, minimal login/MFA panel, Tenant Admin school creation/management/report/settings scaffolds, School Admin parent-link and parent-leave scaffolds, Parent leave-request scaffold, School Admin staff provisioning scaffold, School Admin academic setup scaffold, School Admin academic assignment scaffold, School Admin student import/login scaffold, School Admin bulk jobs scaffold, School Admin fee lifecycle scaffold, School Admin report exports scaffold, tests | Present: `frontend/package.json`, `vite.config.ts`, `tsconfig.json` | Runnable React/Vite baseline with typed onboarding, invitation, login, MFA verification, current-user hydration, allowed-school loading, active-school activation, protected role panels, Tenant Admin school creation, Tenant Admin school management/admin management, Tenant Admin combined report summary/drilldown, Tenant Admin settings/usage panel, parent-linking, parent leave request, staff provisioning, academic setup, academic assignment, student import, queued student import jobs, student login invitation, bulk job, fee demand/payment, and report export API clients. Super Admin onboarding, Tenant Admin school creation/management/report/settings, parent linking, parent leave request/decision, staff provisioning, academic setup, academic assignment, student import, queued student import jobs, student login invitation, bulk jobs, fee lifecycle, and report export actions send the stored Bearer token and show login-required states when missing. Access and refresh tokens are stored in `sessionStorage` for the scaffold only. Full dashboards and mobile auth are not rebuilt yet. | Required now: implement only the next approved task. |
...
| Infrastructure | Docker, local staging compose, root local/staging/prod compose files, Nginx, env templates, monitoring alert rules, backup/load-test placeholders, deployment runbooks, EC2 bootstrap/deploy scripts | Backend/frontend Dockerfiles, local PostgreSQL-backed staging compose, root compose files, frontend Nginx proxy config, host Nginx reverse-proxy template, `.env*example` templates, smoke script, Prometheus alert-rule scaffold, local PostgreSQL backup/restore drill scripts, and EC2 helper scripts exist | Repo-level staging/deployment scaffold exists, builds locally, runs backend against PostgreSQL 16, has a local backup/restore drill that restored 45 public tables, validates root compose files, and has EC2 Docker MVP runbooks/scripts. No real hosted staging environment, managed production secret store, TLS/domain, managed PostgreSQL/object storage, scheduled managed backups, production RTO/RPO policy, or deployed Prometheus/Grafana/Alertmanager stack exists yet. | Required before pilot: configure real staging target, managed PostgreSQL/secrets/TLS/registry/scheduled backups/alerts, and prove managed restore. |
| CI/CD | `.github/workflows/ci.yml`, `.github/workflows/security.yml`, `.github/workflows/deploy.yml`, `.github/dependabot.yml`, plus `.github/workflows/.gitkeep` | GitHub Actions workflow present for backend, frontend, mobile, ops scaffold, security audit, Docker image build/scan/push, manual staging/prod deploy, and nightly/manual security scans | Current CI runs backend Maven tests, frontend `npm ci`/tests/lint/typecheck/build, mobile `npm ci`/lint/typecheck/tests, ops scaffold validation plus compose config validation, and high/critical npm production audit gates. Security workflow adds PR dependency review, CodeQL, and Trivy filesystem SARIF scanning on PR/nightly/manual. Deploy workflow builds/scans backend/frontend images, optionally pushes to GHCR, and can manually deploy to staging/prod through environment secrets. No hosted staging target or OpenAPI publish exists yet. | Required before pilot: configure GitHub environment secrets, real staging deploy target, OpenAPI publication, scheduled backup/restore workflows, and tune scanner false-positive policy as dependencies mature. |
| Documentation | Master plan plus deployment docs | This Markdown file and `docs/deployment/*` | Single source of truth exists, with deployment runbooks split into dedicated docs linked from the master plan. | Required now: update this file after every approved task. |
...
| Database | PostgreSQL 16/pgvector, Flyway V1 to V93, pooled tables with tenant columns | HISTORICAL PARTIAL. Current scaffold has PostgreSQL-compatible Flyway V1-V27 for tenants, schools, users, invitations, school access, audit logs, refresh/session lifecycle, MFA challenges, parent-child linking, parent leave requests, academic year/class/section lifecycle, student import fields/class-section references, subjects, class-subject assignments, teacher assignments, staff profiles, transactional outbox events, durable generic bulk jobs, student import jobs, student-login profile links, fee demands/payments/receipts, attendance sessions/records, homework assignments/submissions, exams/results, notices, report export jobs/files, tenant school limits, tenant settings, notification delivery records, subscription plans, tenant subscriptions, tenant invoices, AI tenant entitlements, AI request audits, AI knowledge documents, AI retrieval audits, timetable entries, school documents, and website pages. | Required later: keep direct tenant/school scope in every future rebuilt business module. |
...
| Monitoring | Actuator, Micrometer Prometheus, OpenTelemetry/Tempo, Grafana, Loki, Alertmanager | PARTIAL. Current scaffold exposes actuator health/readiness and now has Prometheus alert-rule YAML for backend/frontend availability and backend latency/error-rate signals. Prometheus scrape/export endpoint, dashboards, log aggregation, and Alertmanager are not deployed. | Required before pilot: wire Prometheus/Grafana/Alertmanager, SLOs, log retention, tenant-safe dashboards, and alert routing. |
| CI/CD | GitHub Actions CI, Docker publish, security nightly, OpenAPI publish, DR drill, deploy placeholder | HISTORICAL PARTIAL. Current scaffold now has `ci.yml` for backend/frontend/mobile tests/builds/lint gates plus ops scaffold validation, compose config validation, and high/critical npm audit gates. `security.yml` adds dependency review, CodeQL, and Trivy filesystem scanning; Dependabot is configured. `deploy.yml` now builds/scans backend/frontend images, optionally pushes to GHCR, and can manually deploy staging/production through configured environment secrets. | Required before pilot: configure real staging environment secrets/target, run hosted deploy, add OpenAPI publication, and schedule DR workflow. |
| Deployment | Docker Compose, backend/frontend Dockerfiles, local Nginx TLS proxy, starter Kubernetes YAML | PARTIAL. Current scaffold now has backend/frontend Dockerfiles, root local/staging/prod compose files, local staging compose, frontend Nginx SPA/API proxy config, host Nginx reverse-proxy template, env templates, deployment runbooks, local Docker smoke proof, Ubuntu EC2 bootstrap script, and EC2 compose deploy helper. No hosted production IaC. | Required before pilot: managed DB/object storage/secrets/backups/HTTPS/alerts and hosted staging smoke. |
...
|---|---|---|---|---|---|
| Tenant management | PARTIAL. Current scaffold has SUPER_ADMIN-protected `POST /v1/super-admin/tenants/onboard` with first real school input, TENANT_ADMIN-protected additional-school creation within a durable school limit, Tenant Admin school list/name-edit/deactivate, Tenant Admin school-admin invite/grant/list/resend/revoke workflow for tenant schools, and Tenant Admin tenant settings plus subscription usage summary. Historical broader tenant status/feature/subscription pieces are not rebuilt yet. | PARTIAL. Super Admin onboarding form and Tenant Admin school creation, School Admin invite, school management, admin management, report, settings, and usage forms exist, send Bearer auth, and show login-required states. Tenant list/detail not rebuilt yet. | MISSING. | PARTIAL. Visible `MAIN` onboarding is removed, onboarding audit exists, `X-Tenant-ID`/school context headers are rejected, only authenticated `SUPER_ADMIN` can create tenants, only authenticated `TENANT_ADMIN` can manage schools/admin grants/settings/usage for their own tenant within current guardrails, same-tenant schools can receive Tenant Admin-created School Admin grants, primary schools cannot be deactivated, and last School Admin access grant cannot be revoked. | Tenant list/detail, subscription assignment UI/API, plan catalog, invoices/billing, full Super Admin/Tenant Admin route shell, SMTP/provider configuration hardening for invitations. |
| Authentication | PARTIAL. Invitation token hash, expiry, BCrypt set-password, user activation, signed stateless access token login, refresh-token rotation, logout/access-token revocation, forgot/reset password, change password, `/v1/me`, `/v1/me/schools`, school activation, privileged-user MFA challenge verification, and in-process login rate limiting exist. Device/session management and production out-of-band MFA delivery are not rebuilt yet. | PARTIAL. Invitation acceptance, login/MFA slide-over access, auth state provider, current-user and school hydration, logout, role-guarded panels, and school activation UX exist. Auth API client supports login, MFA verification, refresh, logout, reset, change-password, current user, school list, and activation. Full dashboards and production-grade session UX are not rebuilt yet. | PARTIAL. Mobile shell only. | PARTIAL. Secure invitation acceptance, server-derived login identity, refresh rotation, logout revocation, privileged MFA challenge/reuse checks, login rate limiting, and frontend route denial states exist; real second-factor delivery is scaffold-only. | Out-of-band MFA delivery, device-session management, full dashboards, mobile auth. |
| User and roles | PARTIAL. Roles: `SUPER_ADMIN`, `TENANT_ADMIN`, `SCHOOL_ADMIN`, `TEACHER`, `FINANCE_STAFF`, `STAFF`, `PARENT`, `STUDENT`. | PARTIAL. Web role type now matches backend roles and tests lock the list. Tenant Admin is recognised; Finance Staff is recognised and routed to finance-only fee/payment/receipt/report scaffolds. | PARTIAL. Mobile shell model now includes `FINANCE_STAFF`; full mobile finance screens are not rebuilt. | PARTIAL. Finance Staff has school-scoped fee demand/payment access without academic admin access in current scaffold tests. | Broaden permission matrix beyond current fee finance scope. |
| School access | VERIFIED for current backend scaffold. Current scaffold grants first School Admin `user_school_access` during onboarding, has `SchoolAccessService.requireSchoolAdminAccess` to deny unassigned same-tenant and cross-tenant schools, supports `/v1/me/schools/{schoolId}/activate`, filters malformed cross-tenant grants from initial active-school selection, adds Tenant Admin school creation and school-admin grant workflow without trusting frontend tenant ids, and enforces object-school checks in rebuilt parent, staff provisioning, academic, assignment, student-import, bulk-job, fee, attendance, homework, exam/result, notice, report export, timetable, document, website, and AI knowledge routes. | PARTIAL. Web scaffold loads `/v1/me/schools`, auto-activates a single assigned school, lets multi-school users select and activate a school, updates token/session context from the backend response, shows activation-denied errors, and includes minimal Tenant Admin create-school and invite-School-Admin panels. | PARTIAL. Mobile shell role model now includes backend role codes and school-switching readiness markers; full mobile auth/switching screens are not rebuilt. | VERIFIED for current backend routes. Cross-school denial is tested at service guard, activation API, parent-child, staff provisioning, academic lifecycle, academic assignment, student-import, bulk-job, fee, attendance, homework, exam/result, notice, report export, timetable, document, website, and AI retrieval route levels; Tenant Admin school creation and School Admin grants derive tenant from the authenticated user and block spoofing headers; MUL-003 adds explicit multi-school switch and malformed grant tests. | Richer school switching UX, future controller/object-route integration tests as new modules return. |
...
| Academic year | VERIFIED for current scaffold foundation. `AcademicYear` entity, repository, service, controller, activation lifecycle, audit events, and route tests exist. Historical broader academic settings are not rebuilt. | VERIFIED for current scaffold UX. School Admin setup lists existing years, shows current year, creates years with normal fields, and uses selected years to drive class setup. | MISSING. | VERIFIED for current routes. Creation/listing use authenticated active school; activation/class/section/assignment flows load the object's actual school and verify `user_school_access`; cross-school object use is denied in tests. | Pagination, student enrollment links, promotions. |
| Classes and sections | VERIFIED for current scaffold foundation. `ClassLevel`, `Section`, `Subject`, `ClassSubjectAssignment`, and `TeacherAssignment` entities, repositories, services, controllers, DB uniqueness, and route tests exist. | VERIFIED for current scaffold UX. Class creation uses an academic-year selector, section creation uses a class selector, existing records are shown, and raw UUID inputs are hidden from School Admin users. | MISSING. | VERIFIED for current assignment routes. Class creation is scoped to the accessed academic year's school; section creation is scoped to the accessed class's school; class-subject and teacher assignment use object-school checks. | Pagination, student enrollment links, timetable integration. |
| Subjects | VERIFIED for current scaffold foundation. Subject CRUD/list and class-subject assignment APIs exist with object-school checks. | VERIFIED for current scaffold UX. Subject setup lists subjects and uses normal subject code/name fields; class-subject and teacher assignment use academic-year, class, subject, class-subject, and searchable teacher selectors. | MISSING. | VERIFIED for current routes. Subject create/list use active school; class-subject assignment verifies class and subject actual schools. | Full subject catalog edit/delete lifecycle, timetable integration. |
| Staff | PARTIAL. Current scaffold has `staff_profiles`, `POST /v1/school-admin/staff/provision`, portal-login-required `STAFF`/`TEACHER`/`FINANCE_STAFF` user creation/reuse, invitation creation, and school access grants. Historical full staff CRUD, attendance, leave, departments/designations lifecycle are not rebuilt. | PARTIAL. Minimal School Admin staff provisioning scaffold exists and can invite Finance Staff. | PARTIAL. Mobile shell role model recognises Finance Staff only. | VERIFIED for current provisioning route. Uses active authenticated school and `SchoolAccessService`; rejects unsafe roles, profile-only requests, duplicate employee numbers, and non-School Admin callers. | Full staff CRUD/edit, attendance, leave, multi-school staff assignment, broader finance permissions. |
...
| Parent | PARTIAL. Current scaffold has `POST /v1/school-admin/parent-links`, parent invitation creation, `ParentStudentLink`, `GET /v1/parent/children`, `GET /v1/parent/children/{studentId}`, parent leave request create/list APIs, School Admin parent leave list/decision APIs, parent linked-child fee list/payment endpoints, parent linked-child homework listing, and parent linked-child published-result listing. | PARTIAL. Minimal School Admin parent-link form, School Admin leave-decision scaffold, and Parent leave-request scaffold exist; full parent portal/dashboard not rebuilt. | PARTIAL. Mobile shell only. | PARTIAL. Parent provisioning by email with invitation, linked-child-only access, linked-child leave request creation/listing, School Admin active-school leave decisions, linked-child fee/payment access, linked-child homework visibility, and linked-child published result visibility are verified for current scaffold; child detail/dashboard modules are not rebuilt. | Parent dashboard, richer payments UI, multi-child UX, parent homework/result UI, leave notifications/calendar integration. |
...
| Leave | PARTIAL. Current scaffold has parent linked-child leave requests and School Admin active-school leave decisions. Historical staff/teacher leave and broader leave management are not rebuilt. | PARTIAL. Minimal parent request and School Admin decision scaffolds exist. | MISSING. | PARTIAL/VERIFIED for current parent leave routes. Parent routes require `ParentStudentLink`; School Admin routes require `SchoolAccessService` against active/object school; tests deny unlinked-child requests and cross-school School Admin decisions. | Staff/teacher leave, notifications, calendars, attachments, reporting, richer UX. |
...
| Fees and payments | VERIFIED for current scaffold foundation. `fee_demands` and `fee_payments` support School Admin demand creation/list/read/payment, Finance Staff finance-only demand/payment aliases, parent linked-child fee list/payment, student own-fee list, receipt numbers, paid/partial status, and audit events. Historical fee categories/structures, concessions, refunds, reconciliation, Razorpay/webhooks, and finance reports are not rebuilt. | PARTIAL. Minimal School Admin and Finance Staff fee lifecycle scaffolds exist for demand creation and payment recording; parent/student payment UI is not rebuilt. | PARTIAL. Mobile shell role recognition only; fee screens are not rebuilt. | VERIFIED for current routes. School Admin and Finance Staff demand/payment routes verify active school or the object's actual `school_id`; parent routes require linked child; student route resolves the authenticated linked profile; tests deny cross-school demand create/read/pay, unlinked parent access, mismatched child-demand payment, overpayment, and Finance Staff academic-admin access. | Fee category/structure setup, concessions/refunds, reconciliation, webhooks ops, finance reports, parent/student portal UX. |
...
| Timetable | VERIFIED for current School Admin foundation. V27 adds `timetable_entries`; School Admin can create/list/read entries for active-school class/section/subject objects. Teacher/student timetable views are not rebuilt. | MISSING for timetable UX in current scaffold. | MISSING. | VERIFIED for current routes. Object-school checks reject School B class/section/subject ids and School Admin A cannot read School B entries. | Teacher/student timetable views, recurring schedules, room management, substitution workflow, frontend/mobile UX. |
| Documents | VERIFIED for current School Admin metadata foundation. V27 adds `school_documents`; School Admin can create/list/read document metadata scoped to active school, optional class, and optional student. Binary storage/upload/signed URLs are not rebuilt. | MISSING for document UX in current scaffold. | MISSING. | VERIFIED for current metadata routes. Object-school checks reject School B class/student ids and School Admin A cannot read School B documents. | Object storage, upload scanning/quarantine, signed URL authorization, retention rules, parent/student views. |
| Website builder/public website | VERIFIED for current School Admin page foundation. V27 adds `website_pages`; School Admin can create/list/read draft pages and publish same-school pages. Public read/custom domains are not rebuilt. | MISSING for website builder UX in current scaffold. | MISSING. | VERIFIED for current admin routes. School Admin A cannot read or publish School B pages; publish writes audit rows. | Tenant-safe public read APIs, custom domains, branding, rich editor, frontend builder UX. |
| Subscription | VERIFIED for current backend foundation. Current scaffold has durable `tenant_school_limits`, Super Admin `GET/POST/PATCH /v1/super-admin/subscriptions/plans`, `GET/PUT /v1/super-admin/subscriptions/tenants/{tenantId}`, `GET /v1/super-admin/subscriptions/tenants/{tenantId}/invoices`, V23 `subscription_plans`, `tenant_subscriptions`, and `tenant_invoices`, plus `GET /v1/tenant-admin/subscription/usage` reading the assigned plan code and school limit. Tenant Admin school-count enforcement is updated when a Super Admin assigns a plan. Broader feature entitlements, payment collection, invoice PDF/payment status, student/staff limit enforcement, and billing provider integration are not rebuilt yet. | PARTIAL. Tenant Admin school creation displays returned school usage, and the Tenant Admin settings panel displays subscription usage. Super Admin plan/subscription UI is not rebuilt yet. Upgrade page and feature-gated nav are not rebuilt. | PARTIAL. Feature list stored historically only. | PARTIAL. School-count limits are enforced for Tenant Admin school creation and now derive from Super Admin-assigned plans; Super Admin subscription APIs require authenticated `SUPER_ADMIN`; unauthenticated, non-super roles, body role spoofing, unsafe plan downgrades, and spoofed tenant headers are tested. Student/staff/feature limits are not rebuilt yet. | Super Admin subscription UI, feature entitlements, student/staff/feature limit enforcement, invoice payment collection/status, invoice PDFs, billing provider/webhooks, Tenant Admin upgrade UX. |
...
| Notifications | VERIFIED for current invitation-email scaffold. V22 adds `notification_deliveries`; invitation creation paths queue a delivery row and token-safe `InvitationEmailDeliveryRequested` outbox event, then deliver through configurable modes: `log` by default, `smtp` via `JavaMailSender`, or `disabled`. Current invitation email coverage includes Super Admin onboarding School Admin invites, Tenant Admin School Admin invites/resends, parent invites, staff/teacher invites, and student-login invites. Raw invitation tokens are used only in-memory for the SMTP email body and are not persisted in delivery rows or outbox payloads. | PARTIAL. Existing web invitation/onboarding screens still display API-returned scaffold invitation links/tokens for local testing; no notification log UI is rebuilt. | PARTIAL. Device registration API possible later. | PARTIAL. Delivery status persistence and token-safe outbox metadata are verified; no dispatcher retry worker, bounce handling, provider webhooks, template management, push/SMS/WhatsApp, or notification preference model exists yet. | SMTP secrets/provider config, retry/backoff worker, bounce/webhook handling, notification templates, delivery-log UI, user preferences, push/SMS/WhatsApp. |
| AI/RAG if present | VERIFIED for first-version backend AI scope. V25 adds tenant-level AI entitlements and request-audit records; V26 adds school-scoped AI knowledge documents and retrieval audits. Super Admin can enable/disable tenant features, set monthly unit budget, set human-approval requirement, and configure retention days; authenticated usage audit and first-version knowledge retrieval records are server-derived from Bearer identity, active school, teacher grant, parent-child link, or student profile. No live model gateway, embeddings, vector store, or provider execution is implemented yet. | MISSING for AI admin/usage/retrieval UI in the current scaffold. Historical Super Admin AI pages are deleted with the legacy app. | MISSING. | VERIFIED for first-version AI backend. AI-001 verifies Super Admin-only entitlement management, unauthenticated/non-super denial, tenant-header/body spoof resistance, disabled-feature denial, budget-overage denial, and raw-prompt-free audit metadata. AI-002 verifies school/role scoped first-version retrieval, body `schoolId` spoof resistance, parent linked-child scope, student own-profile scope, teacher active-school grant scope, knowledge-document visibility controls, disabled-entitlement denial, and raw-query-free retrieval audit metadata. | Live model gateway, vector/embedding RAG, prompt-template governance, AI feedback, provider secrets, production model execution audit, UI/mobile AI workflows. |
| Audit log | VERIFIED for current scaffold mutation coverage. Current scaffold persists onboarding audit events for tenant creation, school creation, School Admin invitation, school access grant, Tenant Admin additional-school creation, Tenant Admin-created School Admin invitation/access grant, Tenant Admin school updates/deactivation/admin resend/revoke, Tenant Admin settings update, Super Admin subscription plan create/update, tenant subscription assignment, tenant invoice issuance, AI entitlement update, AI usage audit/denial, AI knowledge document creation, AI retrieval audit/denial, invitation acceptance, MFA challenge create/verify, refresh rotation, logout, password reset request/completion, password change, school context activation, parent invitation, parent-child link, staff invitation, staff profile creation, academic year creation/activation, class creation, section creation, subject creation, class-subject assignment, teacher assignment, student import, student import job queueing, student login enablement/invitation, bulk job creation/cancellation, fee demand creation, fee payment recording, receipt issue, attendance submission, homework publish, homework submission, exam creation, exam marks recording, exam result publication, notice creation, notice publication, report export request, report export completion, timetable entry creation, document creation, website page creation, and website page publication with authenticated actor id and role. Historical wider audit viewer/services are not rebuilt yet. | MISSING. | MISSING. | VERIFIED for current scaffold. `AuditCoverageMatrixTest` fails future mutation controllers not added to the matrix and verifies mapped services call `auditLogService.record` with typed actions. Raw invitation tokens, raw refresh/access tokens, reset tokens, MFA codes, passwords, raw AI prompts, raw AI retrieval queries, raw AI knowledge content, student names in import rows, raw student invitation tokens, bulk input/error file references, raw payment gateway references, individual attendance-status details, homework instructions, homework submission content, exam titles, raw marks values, notice titles, notice bodies, timetable notes, document external references, website page content, report file names supplied by users, generated report content, spoofed tenant ids from school-creation/settings/subscription-assignment/AI-entitlement requests, raw Tenant Admin-created School Admin invitation tokens, and raw tenant billing/support email addresses in settings audit metadata are not stored in verified audit metadata. | Audit viewer/API, production delivery logs, failed-login/security telemetry. |
...
| CF-005 | School access enforcement is route-shape dependent. | Historical deleted-code evidence: `SchoolPathAccessInterceptor` only covered `/v1/school-admin/schools/*` paths. Current scaffold evidence: PAR-001 parent-child routes, PAR-002 parent leave routes, STAFF-001 staff/teacher/finance provisioning route, ACA-001 academic year/class/section routes, ACA-002 subject/class-subject/teacher assignment routes, STU-001 student import/list routes, BULK-001 bulk-job routes, STU-003 student login/self-profile routes, FEE-001 school-admin fee demand/payment/receipt routes, FEE-002 finance fee demand/payment routes, ATT-001 attendance routes, HOME-001 homework routes, EXAM-001 exam/result routes, NOTE-001 notice routes, REP-001 report export routes, timetable routes, document routes, website page routes, and AI knowledge routes are guarded and tested. `SchoolScopedControllerGuardCoverageTest` fails any future school-scoped controller without an approved backend guard marker. | Current scaffold route-level school isolation is verified across rebuilt school-scoped modules. Future modules can still reintroduce risk if they bypass the central guard or object-school checks. | Keep `SchoolAccessService` as the central guard for School Admin/Finance Staff routes; parent/student routes must prove parent-child linkage or own profile; teacher/staff routes must prove assignment/object school. Add route-level negative tests when each new business controller is rebuilt. | COMPLETE for current scaffold; regression protected. |
| CF-006 | `TENANT_ADMIN` backend role exists but Tenant Admin product surface is incomplete. | Backend enum includes `TENANT_ADMIN`; web `UserRole` includes it; MUL-002 adds `POST /v1/tenant-admin/schools` and a minimal protected web school-creation scaffold with plan-limit enforcement; MUL-004 adds `GET /v1/tenant-admin/reports/summary` and `GET /v1/tenant-admin/reports/schools/{schoolId}/summary` plus a minimal protected web reports scaffold; MUL-005 adds `POST /v1/tenant-admin/schools/{schoolId}/admins/invite`; MUL-006 adds `GET /v1/tenant-admin/schools`, `PATCH /v1/tenant-admin/schools/{schoolId}`, `POST /v1/tenant-admin/schools/{schoolId}/deactivate`, `GET /v1/tenant-admin/schools/{schoolId}/admins`, `POST /v1/tenant-admin/schools/{schoolId}/admins/{userId}/resend-invitation`, and `DELETE /v1/tenant-admin/schools/{schoolId}/admins/{userId}/access` plus minimal web management forms; MUL-007 adds `GET/PATCH /v1/tenant-admin/settings` and `GET /v1/tenant-admin/subscription/usage` plus a minimal web settings/usage panel; NOTIF-001 adds SMTP-capable invitation email delivery for Tenant Admin-created School Admin invites/resends; SUB-001 adds Super Admin plan catalog, tenant subscription assignment, invoice scaffold, and plan-backed school-limit enforcement. Remaining richer portal navigation, provider delivery hardening, payment collection/reconciliation, and polished dashboard UX are absent. | Multi-school organisation admin now has guarded school creation, school list/name-edit/deactivate, School Admin grant/invitation/list/resend/revoke, combined reporting, tenant settings update, subscription usage visibility, Super Admin-assigned plan limits, and an invitation email delivery foundation, but cannot use a polished organisation portal or billing payment workflow yet. | Required before multi-school release: add Tenant Admin portal shell/navigation, Super Admin subscription UI, payment collection/reconciliation, SMTP/provider delivery hardening, and richer school switching/dashboard UX. | PARTIAL, P1 |
| CF-007 | Accountant/finance staff is not a login role. | Current scaffold evidence: backend `UserRole` now includes `FINANCE_STAFF`; staff provisioning can invite Finance Staff; `/v1/finance/fees/demands` and `/v1/finance/fees/demands/{demandId}/payments` allow fee work through `SchoolAccessService.requireSchoolFinanceAccess`; tests verify Finance Staff can manage assigned-school fees and cannot access academic admin or another school's fee demand. | Current scaffold no longer requires broad School Admin rights for fee demand/payment work. Broader accounting, reconciliation, refund, ledger, and finance report permissions are still future work. | Required before paid commercial release: broaden finance permission matrix and reporting/accounting workflows beyond current fee routes. | COMPLETE for current scaffold finance-only fee scope, P1 |
...
| CF-014 | Frontend role model is out of sync with backend. | Current web scaffold evidence: `frontend/src/features/auth/api/authApi.ts` exports `BACKEND_USER_ROLES` with `SUPER_ADMIN`, `TENANT_ADMIN`, `SCHOOL_ADMIN`, `TEACHER`, `FINANCE_STAFF`, `STAFF`, `PARENT`, and `STUDENT`; `authApi.test.ts` locks the list. Mobile shell model also recognises these backend role codes. | Current web and mobile shell role lists no longer drift for backend-defined roles. Full mobile auth/portal screens still remain future work. | Required before pilot: keep web/mobile role tests updated as new backend roles are introduced. | COMPLETE for current web/mobile shell scaffold, P1 |
...
| CF-016 | Frontend lint gate is absent in the current scaffold. | Current scaffold evidence: frontend and mobile have ESLint flat configs, `npm run lint` scripts, lockfile-pinned lint dependencies, and CI lint steps. Local frontend and mobile lint commands pass. | Static-analysis regressions are now gated for web and mobile scaffold code. Backend static analysis and formatting policy remain future hardening. | Required before pilot: add backend static analysis/formatting and optional stricter lint rules as the codebase matures. | COMPLETE for current frontend/mobile scaffold, P2 |
...
Current code roles: backend, web frontend, and mobile shell model currently have `SUPER_ADMIN`, `TENANT_ADMIN`, `SCHOOL_ADMIN`, `TEACHER`, `FINANCE_STAFF`, `STAFF`, `PARENT`, `STUDENT`.
...
| School/Academic | School, SchoolSettings, AcademicYear, ClassLevel, Section, Subject, ClassSubjectAssignment, TeacherAssignment, Timetable | Current scaffold verifies school, academic year, class, section, subject, class-subject assignment, teacher assignment, and School Admin timetable entry models. Historical settings breadth and teacher/student timetable views are not rebuilt. | PARTIAL | Required before pilot: richer timetable UX, teacher/student timetable views, and broader school access tests as modules return. |
...
Current CI/CD: the current scaffold has `.github/workflows/ci.yml`, which runs backend, frontend, mobile, ops, lint, and high/critical npm audit validation on push, pull request, and manual dispatch. `.github/workflows/security.yml` adds PR dependency review plus nightly/manual CodeQL and Trivy filesystem scanning. It intentionally does not deploy, publish images, publish OpenAPI, or run disaster-recovery drills yet.
...
| STRUCT-002 | VERIFIED | `backend/pom.xml`, `CloudCampusApplication`, readiness API, JPA/Flyway baseline, and tests compile/run. | `frontend/package.json`, Vite app shell, app test, onboarding/invitation screens compile/build. | Flyway V1-V2 validate and apply in H2 test runtime. | Backend 12 tests pass; frontend 4 tests pass; mobile 1 test passes. Current frontend/mobile lint gates are covered by OPS-001. | `cd backend && mvn -q test`; `cd frontend && npm run build`; `cd frontend && npm test`; `cd mobile && npm run typecheck`; `cd mobile && npm test`; OPS-001 adds `cd frontend && npm run lint` and `cd mobile && npm run lint`. | VERIFIED | No backend static-analysis rule exists yet. | CI-001 now gates scaffold build/typecheck/tests; OPS-001 gates frontend/mobile lint. |
...
| SEC-002 | VERIFIED | `SchoolAccessService.requireSchoolAdminAccess` allows explicitly granted school and rejects unassigned same-tenant and cross-tenant schools. PAR-001 parent-child routes, STAFF-001 staff/teacher provisioning route, ACA-001 academic year/class/section routes, ACA-002 subject/class-subject/teacher assignment routes, STU-001 student import/list routes, BULK-001 bulk-job routes, STU-003 student login/self-profile routes, FEE-001 fee demand/payment routes, ATT-001 attendance routes, HOME-001 homework routes, EXAM-001 exam/result routes, NOTE-001 notice routes, REP-001 report export routes, timetable routes, document routes, website page routes, and AI knowledge routes enforce route-level object school checks in the current scaffold. | Minimal parent-link, staff provisioning, academic setup, academic assignment, student import, bulk job, fee lifecycle, backend attendance, backend homework, backend exam/result, backend notice, backend report export, backend timetable, backend document metadata, and backend website page scaffolds exist; no full polished School Admin portal or cross-school manipulation UI exists. | V1 has `user_school_access`; V5 adds parent-child school-linked student anchors; V6 adds academic school-scoped tables; V7 adds student class/section import fields; V8 adds subjects, class-subject assignments, and teacher assignments; V9 adds staff profiles; V11 adds bulk jobs; V13 links student login users; V14 adds fee demand/payment tables; V15 adds attendance sessions/records; V16 adds homework assignments/submissions; V17 adds exams/results; V18 adds notices; V19 adds report export jobs/files; V26 adds AI knowledge documents; V27 adds timetable entries, school documents, and website pages. | `SchoolAccessIsolationTest` proves the guard at service level and parameterizes same-tenant denial across object types. `ParentChildLinkingFlowTest`, `StaffProvisioningFlowTest`, `AcademicLifecycleFlowTest`, `AcademicAssignmentFlowTest`, `StudentImportFlowTest`, `StudentImportJobFlowTest`, `StudentLoginProvisioningFlowTest`, `BulkJobFlowTest`, `FeeLifecycleFlowTest`, `AttendanceFlowTest`, `HomeworkFlowTest`, `ExamFlowTest`, `NoticeFlowTest`, `ReportExportFlowTest`, `AiScopedRetrievalFlowTest`, and `SchoolScopedMissingModulesFlowTest` prove route-level object denial for current rebuilt modules. `SchoolScopedControllerGuardCoverageTest` verifies school-scoped controllers have approved guard markers. | `cd backend && mvn -q -Dtest=SchoolScopedMissingModulesFlowTest,SchoolScopedControllerGuardCoverageTest,AuditCoverageMatrixTest test`; `cd backend && mvn -q test`; `rg -n "@RestController|@Controller|@RequestMapping|@GetMapping|@PostMapping|@PutMapping|@PatchMapping|@DeleteMapping" backend/src/main/java`; `find backend/src/main/java/com/cloudcampus -type d` | VERIFIED | Parent/student attendance routes, public website reads, signed document storage, and full frontend/mobile UX are not rebuilt yet; each future route must keep adding route-level negative tests. | If future school-scoped APIs skip central guard/object-school checks, cross-school exposure can be reintroduced despite current scaffold protection. |
...
| `TimetableController` | School Admin timetable entry create/list/read. | Create/list use authenticated active school; create verifies class/section/subject actual schools; read loads the timetable entry object's actual school and verifies `SchoolAccessService.requireSchoolAdminAccess`; no frontend school id is trusted. |
| `SchoolDocumentController` | School Admin school document metadata create/list/read. | Create/list use authenticated active school; create verifies optional class/student actual schools; read loads the document object's actual school and verifies `SchoolAccessService.requireSchoolAdminAccess`; no frontend school id is trusted. Binary storage and signed URLs are not in this first foundation. |
| `WebsiteController` | School Admin website page create/list/read/publish. | Create/list use authenticated active school; read/publish load the page object's actual school and verify `SchoolAccessService.requireSchoolAdminAccess`; no frontend school id is trusted. Public website reads are intentionally not rebuilt yet. |
...
| Timetable | V27 current scaffold APIs exist for School Admin timetable entry create/list/read. Teacher/student timetable views are not rebuilt. | `SchoolScopedMissingModulesFlowTest` proves School Admin A cannot create entries using School B class/section/subject ids, cannot read School B timetable entries, and can list/read own-school entries. `SchoolScopedControllerGuardCoverageTest` includes `TimetableController`. | Add teacher/student assignment-school route tests, recurrence, substitutions, and frontend/mobile UX when timetable scope expands. |
| Documents | V27 current scaffold APIs exist for School Admin school document metadata create/list/read. Binary upload/signed URL delivery is not rebuilt. | `SchoolScopedMissingModulesFlowTest` proves School Admin A cannot create documents using School B class/student ids, cannot read School B documents, and can list/read own-school documents. `SchoolScopedControllerGuardCoverageTest` includes `SchoolDocumentController`. | Add object storage, malware scanning/quarantine, signed URL authorization, retention, parent/student views, and frontend/mobile UX when document scope expands. |
...
| Website content | V27 current scaffold APIs exist for School Admin website page create/list/read/publish. Public website read/custom domain delivery is not rebuilt. | `SchoolScopedMissingModulesFlowTest` proves School Admin A cannot read or publish School B website pages, own-school publish works, and website audit actions are persisted. `SchoolScopedControllerGuardCoverageTest` includes `WebsiteController`. | Add tenant-safe public read APIs, custom domains, cache invalidation, branding, rich editor, and frontend builder UX when website scope expands. |

Files changed for SEC-006:

- `backend/src/main/java/com/cloudcampus/audit/AuditAction.java`
- `backend/src/main/java/com/cloudcampus/operations/timetable/*`
- `backend/src/main/java/com/cloudcampus/operations/document/*`
- `backend/src/main/java/com/cloudcampus/operations/website/*`
- `backend/src/main/resources/db/migration/V27__timetable_document_website_foundation.sql`
...
SEC-006 result: VERIFIED for current scaffold route-level product readiness. Current parent-child, academic lifecycle, academic assignment, teacher-assignment, student-import, student-login, bulk-job, fee, attendance, homework, exam/result, notice, report export, timetable, document metadata, website page, and AI knowledge routes are guarded, and a regression test fails future unguarded school-scoped controllers. `SEC-002` is also VERIFIED for current scaffold school isolation, with future expansion required for new routes as they are rebuilt.

SEC-006 update after EXAM-001: current scaffold guard coverage added guarded exam/result routes as well as attendance and homework. At that checkpoint, `SEC-002` remained PARTIAL because notices, timetable, documents, reports, website content, parent/student attendance, and broader route surfaces still needed route-level object isolation tests when rebuilt.

SEC-006 update after NOTE-001: current scaffold guard coverage added guarded notice routes. At that checkpoint, `SEC-002` remained PARTIAL because timetable, documents, reports, website content, parent/student attendance, tenant/organisation notices, and broader route surfaces still needed route-level object isolation tests when rebuilt.

SEC-006 update after MUL-004: current scaffold guard coverage added guarded report export routes and Tenant Admin combined-report school drilldown denial. At that checkpoint, `SEC-002` remained PARTIAL because timetable, documents, website content, parent/student attendance, tenant/organisation notices, and broader route surfaces still needed route-level object isolation tests when rebuilt.

SEC-006 update after V27 timetable/document/website foundation: current scaffold guard coverage now includes guarded School Admin timetable, document metadata, and website page routes. `SchoolScopedMissingModulesFlowTest` covers School Admin A denied from School B timetable entries, documents, and website pages; verifies cross-school class/section/subject/student inputs are rejected; and verifies timetable/document/website audit actions. Parent/student attendance routes, public website reads, signed document storage, and richer frontend/mobile surfaces remain future work, but no current rebuilt school-scoped backend controller is left without route-level school isolation evidence.
...
## 25IA. PAR-002 Verification Notes

Verification run on 2026-05-27:

- `cd backend && mvn test -Dtest=ParentLeaveRequestFlowTest,AuditCoverageMatrixTest,SchoolScopedControllerGuardCoverageTest`
- `cd frontend && npm test -- --run src/features/parent/pages/ParentLeaveRequestsPage.test.tsx src/features/parent/pages/SchoolAdminLeaveRequestsPage.test.tsx src/features/parent/pages/SchoolAdminParentLinkPage.test.tsx`

APIs added:

| API | Behavior | Security source of truth |
|---|---|---|
| `POST /v1/parent/children/{studentId}/leave-requests` | Parent creates a leave request for a linked child. | Bearer token user must be `PARENT`; the requested student must be linked through `ParentStudentLink`; tenant/school are derived from the link and student, not request input. |
| `GET /v1/parent/children/{studentId}/leave-requests` | Parent lists leave requests for a linked child. | Parent-child link lookup by authenticated parent user id and student id. |
| `GET /v1/school-admin/parent-leave-requests` | School Admin lists leave requests for the active school. | Active school comes from authenticated token context and is verified with `SchoolAccessService.requireSchoolAdminAccess`. |
| `PATCH /v1/school-admin/parent-leave-requests/{leaveRequestId}` | School Admin approves or rejects a pending leave request. | Object's actual `school_id` is loaded from the leave request and verified with `SchoolAccessService`; frontend school ids are not trusted. |

Files changed for PAR-002:

- `backend/src/main/resources/db/migration/V24__parent_leave_requests.sql`
- `backend/src/main/java/com/cloudcampus/audit/AuditAction.java`
- `backend/src/main/java/com/cloudcampus/people/parent/ParentLeaveRequest.java`
- `backend/src/main/java/com/cloudcampus/people/parent/ParentLeaveRequestStatus.java`
- `backend/src/main/java/com/cloudcampus/people/parent/ParentLeaveRequestRepository.java`
- `backend/src/main/java/com/cloudcampus/people/parent/ParentLeaveRequestCreateRequest.java`
- `backend/src/main/java/com/cloudcampus/people/parent/ParentLeaveDecisionRequest.java`
- `backend/src/main/java/com/cloudcampus/people/parent/ParentLeaveRequestResponse.java`
...
- `frontend/src/features/parent/pages/ParentLeaveRequestsPage.tsx`
- `frontend/src/features/parent/pages/ParentLeaveRequestsPage.test.tsx`
- `frontend/src/features/parent/pages/SchoolAdminLeaveRequestsPage.tsx`
...
- Linked parent creates a leave request, lists it, and School Admin approves it for the active school.
- Parent cannot create a leave request for an unlinked child.
- School Admin A cannot decide School B leave request.
- Leave request audit rows record typed `PARENT_LEAVE_REQUESTED` and `PARENT_LEAVE_DECIDED` actions without raw reason/admin-note text.
- Frontend Parent leave screen requires a stored parent token, submits the linked-child leave request, and refreshes the list.
- Frontend School Admin leave screen requires a stored School Admin token, loads active-school leave requests, and submits a decision.

Status decision: `PAR-002` is VERIFIED for the current scaffold foundation. It intentionally does not implement staff/teacher leave, attachments, notification delivery, calendar integration, leave balances, or mobile screens.
...
Status decision: `STAFF-001` is VERIFIED for the current scaffold staff/teacher portal-login provisioning foundation. Finance Staff provisioning and finance-only fee scope are verified separately by FEE-002. STAFF-001 intentionally does not implement full staff CRUD/edit, staff attendance, staff leave, department/designation lifecycle, multi-school staff assignment, production invitation delivery, or a teacher dashboard.
...
## 25TA. FEE-002 Verification Notes

FEE-002 implementation scope:

- Added backend `FINANCE_STAFF` as the current scaffold finance/accountant login role.
- Reused School Admin staff provisioning to invite Finance Staff with portal login, standard invitation/set-password, `user_school_access`, and privileged MFA challenge.
- Added central `SchoolAccessService.requireSchoolFinanceAccess` so finance access is derived from authenticated user identity plus `user_school_access`, not frontend role/school claims.
- Added finance-only fee demand/payment aliases while keeping existing School Admin fee routes backward compatible.
- Added minimal web Finance Staff portal navigation for dashboard, fees, payments, receipts, and finance reports.
- Added mobile shell role recognition for `FINANCE_STAFF`.
- No DB migration was required because roles are persisted as enum strings in existing `user_accounts`, `user_school_access`, `staff_profiles`, and invitation records.

APIs added:

| API | Behavior | Security source of truth |
|---|---|---|
| `POST /v1/finance/fees/demands` | Finance Staff creates a fee demand for a student in the active assigned school. | Bearer token resolves the actor; active school comes from the signed session; `user_school_access` must contain `FINANCE_STAFF` or `SCHOOL_ADMIN`; the student's actual school must match. |
| `GET /v1/finance/fees/demands` | Lists demands for the active assigned finance school. | Active school and school access are server-derived. |
| `GET /v1/finance/fees/demands/{demandId}` | Reads one demand. | Demand's actual `school_id` is loaded and checked with `requireSchoolFinanceAccess`. |
| `POST /v1/finance/fees/demands/{demandId}/payments` | Records a finance staff payment and issues receipt state through existing fee lifecycle. | Demand's actual `school_id` is loaded and checked; payment overage and closed-demand rules remain unchanged. |

Files changed for FEE-002:

- `backend/src/main/java/com/cloudcampus/identity/auth/UserRole.java`
- `backend/src/main/java/com/cloudcampus/identity/auth/session/AuthSessionService.java`
- `backend/src/main/java/com/cloudcampus/identity/accesscontrol/SchoolAccessService.java`
- `backend/src/main/java/com/cloudcampus/people/staff/StaffProvisioningService.java`
- `backend/src/main/java/com/cloudcampus/operations/finance/FeeController.java`
- `backend/src/main/java/com/cloudcampus/operations/finance/FeeService.java`
- `backend/src/test/java/com/cloudcampus/operations/finance/FeeLifecycleFlowTest.java`
...
- `FeeLifecycleFlowTest` verifies Finance Staff can be provisioned, accept invitation, complete MFA-protected login, create/list/read fee demands, record payments through `/v1/finance/...`, and produce audit metadata with actor role `FINANCE_STAFF` without raw payment reference leakage.
- Negative tests verify Finance Staff cannot read, pay, or create fee demand work against another school's student/demand.
- Negative tests verify Finance Staff cannot access academic admin routes such as `/v1/school-admin/academic-years`.
- Frontend tests verify backend role list includes `FINANCE_STAFF`, Staff Provisioning can invite Finance Staff, and Finance Staff sees only the finance area rather than Super Admin or academic setup.
- Mobile tests verify `FINANCE_STAFF` is part of the shell role model and school-switch-ready roles.
- Focused validation commands: `cd backend && mvn -q -Dtest=FeeLifecycleFlowTest test`; `cd backend && mvn -q -Dtest=FeeLifecycleFlowTest,StaffProvisioningFlowTest,AuthSessionFlowTest,TenantOnboardingFlowTest test`; `cd frontend && npm test -- --run App authApi StaffProvisioningPage FeeLifecyclePage`; `cd mobile && npm test -- --run`.
- Full validation commands: `cd backend && mvn -q test`; `cd frontend && npm test -- --run`; `cd frontend && npm run typecheck`; `cd frontend && npm run build`; `cd mobile && npm run typecheck`; `cd mobile && npm test`.

Status decision: `FEE-002` is VERIFIED for the current scaffold finance-only fee demand/payment portal scope. It intentionally does not implement accounting ledger, refunds, payment gateway reconciliation, finance report read models, invoice PDFs, approval workflows, or full mobile finance screens.

## 25TB. OPS-001 Verification Notes

OPS-001 implementation scope:

- Added ESLint flat config to the frontend scaffold.
- Added ESLint flat config to the mobile scaffold.
- Added `npm run lint` scripts for frontend and mobile.
- Added lint dependencies to frontend/mobile package manifests and lockfiles.
- Wired frontend lint and mobile lint into `.github/workflows/ci.yml` before typecheck/build.
- Kept backend static analysis out of scope because OPS-001 was scoped to the missing frontend/mobile lint gate in the current scaffold.

Files changed for OPS-001:
...
- `cd frontend && npm test -- --run` passed with 53 tests.
...
Status decision: `OPS-001` is VERIFIED for the current frontend/mobile scaffold static-analysis gate. It intentionally does not add backend Checkstyle/SpotBugs/Error Prone, Prettier formatting, dependency/security scans, Docker image scans, or staging deployment.

## 25TC. OPS-002 Verification Notes

Task scope: add a repo-level staging/deploy readiness scaffold with secret contract, smoke checks, image manifests, Nginx proxy, alert rules, and CI validation. This does not create a hosted cloud environment.
...
- `scripts/ci/validate-ops.sh`
- `scripts/ops/smoke-staging.sh`
- `docs/CLOUDCAMPUS_MASTER_ARCHITECTURE_AND_EXECUTION_PLAN.md`

Implemented ops scaffold:

- Added backend multi-stage Dockerfile that builds the Spring Boot jar from the repo root context, installs `curl` for container healthchecks, runs as a non-root `cloudcampus` user, and supports runtime `JAVA_OPTS`.
- Added PostgreSQL runtime driver dependency while preserving current H2 dev/test defaults.
- Added Micrometer Prometheus registry dependency and actuator Prometheus exposure configuration.
- Added frontend multi-stage Dockerfile that builds the Vite app from the repo root context and serves it through Nginx.
- Added Nginx SPA config with `/v1/` and `/actuator/health` proxying to the backend service.
- Added local staging compose with backend/frontend build definitions, healthchecks, port mappings, and staging env-file contract.
- Added `infra/env/staging.example.env` with no committed production secrets and Super Admin bootstrap disabled by default.
- Added staging smoke script for backend health, readiness, frontend availability, and optional login/MFA-challenge probe.
- Added Prometheus alert-rule scaffold for backend/frontend availability plus backend error-rate and latency signals.
- Added ops validation script and CI `ops` job that checks required files, blocks committed dev bootstrap password in ops/deploy files, validates shell scripts, and validates staging compose config.

Validation evidence:

- `sh scripts/ci/validate-ops.sh` passed.
- `docker compose -f infra/docker/local/compose.staging.yml config` passed.
- `sh scripts/ops/smoke-staging.sh` passed against the running local backend/frontend.
- `CLOUDCAMPUS_SMOKE_EMAIL=superadmin@cloudcampus.dev CLOUDCAMPUS_SMOKE_PASSWORD='SuperAdmin123!' sh scripts/ops/smoke-staging.sh` passed in local dev, returning a valid login/MFA challenge through the frontend proxy.
- `cd backend && mvn -q test` passed.
- `docker build -f backend/Dockerfile -t cloudcampus-backend:ops002 .` passed. The first draft with Maven `dependency:go-offline` was removed after it proved unnecessarily slow on a cold Docker cache.
- `docker build -f frontend/Dockerfile -t cloudcampus-frontend:ops002 .` passed. Vite reported the existing large chunk warning; route-level code splitting remains a frontend optimization.

Status decision: `OPS-002` is VERIFIED for the current repository-level staging/ops scaffold. Remaining gaps before pilot are a real hosted staging target, managed secret store, production SMTP secret injection, image registry publish, TLS/domain setup, managed PostgreSQL/object storage, Prometheus/Grafana/Alertmanager deployment, and DR workflow. Dependency/security scan gates are covered by OPS-003; local backup/restore proof is covered by OPS-004.

## 25TC1. OPS-003 Verification Notes

Task scope: add first-version security and dependency scan gates without introducing deploy, image publish, or production infrastructure changes.
...
- Added a CI `security-audit` job that runs `scripts/ci/security-audit.sh`.
- Added production dependency audit gates for frontend and mobile using `npm audit --omit=dev --audit-level=high`.
- Added `.github/workflows/security.yml` with PR dependency review, nightly/manual CodeQL, and nightly/manual Trivy filesystem SARIF scanning.
- Added Dependabot weekly update coverage for backend Maven, frontend npm, mobile npm, and GitHub Actions.

Validation evidence:

- `ruby -e "require 'yaml'; YAML.load_file('.github/workflows/ci.yml'); YAML.load_file('.github/workflows/security.yml'); YAML.load_file('.github/dependabot.yml')"` passed.
- `sh scripts/ci/security-audit.sh` passed.
- Frontend production audit reports 0 vulnerabilities.
- Mobile production audit reports 10 moderate Expo transitive advisories, 0 high, and 0 critical; first-version CI blocks high/critical while keeping moderate advisories documented for future Expo dependency upgrades.

Status decision: `OPS-003` is VERIFIED for first-version security/dependency scan gates. CodeQL, Trivy, and dependency-review execution is performed by GitHub Actions; local validation covers workflow syntax and npm high/critical audit gates.

## 25TC2. OPS-004 Verification Notes

Task scope: add a local PostgreSQL backup/restore drill for the repo-level staging scaffold. This does not add hosted managed backups, offsite retention, production RTO/RPO policy, or a scheduled DR workflow.

Files changed:

- `.gitignore`
- `backend/pom.xml`
- `backend/src/main/java/com/cloudcampus/audit/AuditLog.java`
- `backend/src/main/java/com/cloudcampus/events/outbox/OutboxEvent.java`
- `backend/src/main/java/com/cloudcampus/operations/bulk/BulkJob.java`
- `backend/src/main/java/com/cloudcampus/operations/report/ReportExportFile.java`
- `backend/src/main/java/com/cloudcampus/operations/report/ReportExportJob.java`
- `backend/src/main/java/com/cloudcampus/people/student/StudentImportJob.java`
- `backend/src/main/resources/db/migration/V2__audit_logs.sql`
- `backend/src/main/resources/db/migration/V10__transactional_outbox.sql`
- `backend/src/main/resources/db/migration/V11__bulk_jobs.sql`
- `backend/src/main/resources/db/migration/V12__student_import_jobs.sql`
...
Implemented backup/restore foundation:

- Added a PostgreSQL 16 service to local staging compose, with backend startup waiting for PostgreSQL health and conflict-safe host ports.
- Switched staging env defaults from H2 to PostgreSQL while keeping test/dev H2 defaults in `application.yml`.
- Added Flyway PostgreSQL database support and converted large text/json migrations plus matching entity mappings from H2 `CLOB` assumptions to portable `TEXT`.
- Added `scripts/ops/backup-local-postgres.sh` to write a custom-format `pg_dump` and manifest into ignored `infra/backup/local`.
- Added `scripts/ops/restore-local-postgres-drill.sh` to restore a dump into `cloudcampus_restore_drill`, verify public tables exist, and drop the drill database by default.
- Extended ops validation and `.gitignore` so backup scripts are checked and generated dumps/manifests are not committed.

Validation evidence:

- `docker compose -f infra/docker/local/compose.staging.yml up -d --build postgres backend` passed after compatibility fixes.
- Backend health became `healthy` against PostgreSQL 16 after Flyway applied V1-V26 during OPS-004. V27 was added later for timetable/document/website tables and is covered by backend tests.
- `sh scripts/ops/backup-local-postgres.sh` created `infra/backup/local/cloudcampus-cloudcampus-20260527T064121Z.dump` and its manifest.
- `sh scripts/ops/restore-local-postgres-drill.sh` restored the dump into `cloudcampus_restore_drill` and reported `Restore drill passed: restored 45 public tables.`
- `mvn -f backend/pom.xml test` passed with 133 tests, 0 failures, 0 errors, 0 skipped after V27 school-scoped module coverage.
- `sh scripts/ci/validate-ops.sh`, `docker compose -f infra/docker/local/compose.staging.yml config`, and `git diff --check` passed.
- `docker compose -f infra/docker/local/compose.staging.yml down` stopped the local validation stack.

Status decision: `OPS-004` is VERIFIED for a local PostgreSQL backup/restore drill. Remaining production gaps are managed cloud backups, encrypted offsite retention, formal RTO/RPO, scheduled restore drills, a production restore runbook, and hosted staging/production DR automation.
...
## 25UA. FRONT-UX-001 / FRONT-UX-002 Verification Notes

Task scope: improve the authenticated web experience from scaffold-grade to premium SaaS-grade without changing backend auth, tenant, school, or product contracts.
...
- Added `recharts` and role-aware analytics cards for Super Admin, Tenant Admin, School Admin, Teacher, Finance Staff, Staff, Parent, and Student dashboards.
- Added Lucide icon navigation, premium sticky topbar, global search/command palette, notification popover, profile dropdown, theme toggle, school context chip, quick actions button, animated metric cards, and responsive mobile record cards.
- Added a live dashboard header experience with current page title, breadcrumbs, current date, live time, active user, role, active school/platform scope, current-session status, profile session metadata, and role-specific information panels.
- Added role-specific info panels for the requested dashboard checklist: Super Admin users/API health/onboarding/audit/notification signals; Tenant Admin organisation, subscription, school usage, students/teachers, attendance/fees/notices; School Admin school, academic year, roster, attendance, fees, approvals, exams, homework and notices; Teacher schedule/classes/attendance/homework/exams/notices; Finance collection/dues/payments/receipts/failed payments/collection chart; Parent child selector, attendance, homework, fees, notices, exams and leave; Student class, attendance, homework, exams, results, notices and AI study placeholder; plus Staff operations.

Validation evidence:

- `cd frontend && npm test -- --run App` passed with 15 focused app tests, including explicit checks for dashboard header/session information and role-specific Super Admin/School Admin panels.
- `cd frontend && npm test -- --run` passed with 57 tests.
- `cd frontend && npm run lint` passed.
- `cd frontend && npm run typecheck` passed.
- `cd frontend && npm run build` passed. Vite reported the existing large chunk warning after bundling Recharts/framer; code-splitting remains a future production optimization.
- Backend tests were not rerun because no backend code or API contract changed. Mobile typecheck was not rerun because mobile was not touched.

Status decision: `FRONT-UX-001` and `FRONT-UX-002` are VERIFIED for the current web scaffold. Remaining gaps are real backend analytics data wiring, route-level lazy loading/code splitting, deeper School Admin table/form workflows, full mobile-native auth/dashboard UX, and production observability for frontend errors.
...
- Deployment, image publishing, OpenAPI publishing, and DR drills are intentionally deferred. Lint was added in OPS-001 and dependency/security scan gates were added in OPS-003.
...
Status decision: `MUL-002` is VERIFIED for the current scaffold multi-school creation foundation. Super Admin subscription assignment APIs and plan catalog were added later by SUB-001. It intentionally does not implement mobile Tenant Admin UX or production billing reconciliation. Tenant Admin combined summary/drilldown was added later by MUL-004, new-school School Admin invitation/grant workflow was added later by MUL-005, and Tenant Admin school list/edit/deactivate was added later by MUL-006.
...
Status decision: `MUL-006` is VERIFIED for the current scaffold Tenant Admin school-management and School Admin grant-management foundation. Tenant settings and usage were added later by MUL-007; invitation email delivery was added later by NOTIF-001. It intentionally does not implement polished dashboard navigation, provider-grade notification retry/template hardening, invitation cancellation, reactivation, billing-grade subscription assignment, or mobile Tenant Admin UX.
...
Status decision: `MUL-007` is VERIFIED for the current scaffold Tenant Admin tenant settings and subscription usage foundation. Super Admin subscription assignment, plan catalog, and invoice scaffold were added later by SUB-001. It intentionally does not implement feature entitlements, usage over all future modules, organisation branding, polished navigation, payment collection/reconciliation, or mobile Tenant Admin UX.
...
| ARCH-002 | PHASE 0 | Inventory current APIs and roles | P0 | None | Yes | Yes | Yes | No | Route/role inventory | VERIFIED | Current scaffold exposes readiness, onboarding, invitation/auth/session/MFA/password lifecycle, current-user school APIs, parent-child, staff/teacher/finance provisioning, academic lifecycle, academic assignment, teacher assignment, student import, student import job, student login invitation/self-profile, bulk job, school-admin and finance fee demand/payment/receipt, attendance, homework, exam/result, notice, and report export endpoints; backend role enum includes `SUPER_ADMIN`, `TENANT_ADMIN`, `SCHOOL_ADMIN`, `TEACHER`, `FINANCE_STAFF`, `STAFF`, `PARENT`, `STUDENT`; historical route inventory is labelled as rebuild reference. |
...
| FRONT-HOME-001 | PHASE 1 | Add premium public SaaS homepage with universal login access | P1 | FRONT-AUTH-001 | No | Yes | No | No | Frontend homepage/auth tests, build | VERIFIED | Replaced logged-out auth-card shell with a public marketing homepage, sticky blurred header, hero/dashboard/mobile preview, feature/stat/AI/testimonial/pricing/footer sections, and slide-over universal login plus invitation access. Protected role panels remain hidden until authentication. Frontend tests, typecheck, and production build pass. |
| FRONT-UX-001 | PHASE 1 | Upgrade authenticated app shell and dashboards to premium SaaS UX | P1 | FRONT-AUTH-001, FRONT-HOME-001 | No | Yes | No | No | Frontend app tests, lint, typecheck, build | VERIFIED | Added Recharts dependency, Lucide icon navigation, premium sticky topbar, global search/command palette, notification and profile popovers, quick actions, school context chip, animated metric cards, role-specific Recharts analytics, and mobile record-card table behavior. No backend or mobile contracts changed. |
| FRONT-UX-002 | PHASE 1 | Add role-aware dashboard headers and session information panels | P1 | FRONT-UX-001 | No | Yes | No | No | Frontend app tests, lint, typecheck, build | VERIFIED | Added live date/time dashboard header, breadcrumbs, active user/role/school/session status, profile dropdown metadata including current-session last-login placeholder, and role-specific information cards for Super Admin, Tenant Admin, School Admin, Teacher, Finance Staff, Staff, Parent, and Student. Tests verify logged-in header/session behavior and role-specific dashboard info. |
| UX-ACA-FORM-001 | PHASE 1 | Replace raw-ID School Admin academic setup with selector-driven UX | P1 | ACA-001, ACA-002, STAFF-001 | No | Yes | No | No | Frontend academic selector tests, lint, typecheck, build | VERIFIED | `AcademicSetupPage` loads academic years/classes/sections and creates class/section records using selected names instead of visible UUID fields. `AcademicAssignmentsPage` loads academic years/classes/subjects/class-subjects/teachers, adds teacher search, and assigns subjects/teachers with selectors. Tests verify dropdown loading, raw-ID fields absent, correct API payloads, and loading/empty/error states. |
...
| SEC-006 | PHASE 2 | Add object-level school access guard for ID-only routes | P0 | SEC-002 | Yes | No | No | No | Cross-school object route tests | VERIFIED | Current scaffold has guarded school-scoped routes for parent-child access, staff/teacher/finance provisioning, academic lifecycle, academic assignment/teacher assignment, student import/list/job routes, student login provisioning/self-profile routes, bulk jobs, school-admin and finance fee demand/payment/receipt routes, attendance routes, homework routes, exam/result routes, notice routes, report export routes, timetable routes, document metadata routes, website page routes, Tenant Admin report school drilldown, and AI knowledge routes. `SchoolScopedMissingModulesFlowTest` proves School Admin A cannot use School B class/section/subject/student objects or read/publish School B timetable/document/website objects. `SchoolScopedControllerGuardCoverageTest` fails future school-scoped controllers without approved backend guard markers. |
...
| PAR-002 | PHASE 3 | Implement parent leave request flow | P2 | PAR-001 | Yes | Yes | Yes | Maybe | Parent leave tests | VERIFIED | Added V24 `parent_leave_requests`, Parent linked-child leave create/list APIs, School Admin active-school leave list/approve/reject APIs, `PARENT_LEAVE_REQUESTED`/`PARENT_LEAVE_DECIDED` audit events, route-level negative tests for unlinked-child parent access and cross-school School Admin decisions, audit redaction checks for reason/admin notes, and minimal web Parent request and School Admin decision scaffolds. Staff/teacher leave, attachments, notifications, calendars, leave balances, and mobile screens remain follow-ups. |
...
| FEE-001 | PHASE 3 | Complete fee demand/payment/receipt lifecycle | P1 | SEC-006 | Yes | Yes | No | Yes | Payment/receipt/reconciliation tests | VERIFIED | Added V14 `fee_demands` and `fee_payments`, School Admin demand create/list/read/payment APIs, parent linked-child fee list/payment APIs, student own-fee list API, payment status transitions, receipt numbers, fee audit events, cross-school/unlinked-parent/mismatched-demand/overpayment negative tests, and frontend fee lifecycle scaffold. Payment gateway/webhooks, reconciliation, fee setup, parent/student UI, and mobile flows remain follow-ups. Finance role access is covered by FEE-002. |
| FEE-002 | PHASE 6 | Add finance/accountant role and finance-only portal scope | P1 | FEE-001 | Yes | Yes | Yes | No | Role/permission tests | VERIFIED | Added `FINANCE_STAFF` backend/web/mobile role alignment, School Admin staff provisioning support, privileged MFA requirement, `SchoolAccessService.requireSchoolFinanceAccess`, `/v1/finance/fees/demands` and payment routes, minimal Finance Staff portal scope, and tests proving assigned-school fee access, cross-school denial, and academic-admin denial. No migration was required because existing enum-string columns store role codes. |
...
| NOTIF-001 | PHASE 4 | Add invitation email delivery foundation | P1 | AUTH-001, EVT-001 | Yes | No | No | Yes | Invitation email delivery tests | VERIFIED | Added Spring Mail support, env-backed SMTP/local delivery configuration, V22 `notification_deliveries`, and `InvitationEmailDeliveryService` with `log`, `smtp`, and `disabled` modes. Onboarding School Admin invites, Tenant Admin School Admin invites/resends, parent invites, staff/teacher invites, and student-login invites now create delivery rows plus token-safe `InvitationEmailDeliveryRequested` outbox metadata. Tests verify SMTP sends the acceptance URL, local mode masks recipients, delivery rows persist status/provider, and raw invitation tokens/acceptance URLs are not persisted in delivery rows or outbox payloads. Provider secrets, retry/backoff worker, bounce/webhook handling, templates, preferences, and notification UI remain follow-ups. |
...
| OPS-002 | PHASE 6 | Wire staging deploy and production secrets/alerts | P1 | OPS-001 | Yes | Yes | Maybe | No | Deploy smoke tests | VERIFIED | Added backend/frontend Dockerfiles, `.dockerignore`, frontend Nginx SPA/API proxy, local staging compose, staging env contract with no production secrets, Prometheus alert-rule scaffold, smoke script, ops CI validation, and compose config validation. Backend/frontend container images build locally; backend tests and local smoke checks pass. This verifies repo-level staging readiness, not a hosted cloud deployment. Remaining pilot gaps: managed secrets, real staging target, TLS/domain, image registry publish, managed PostgreSQL/object storage, and Alertmanager/Grafana deployment. Dependency/security scan gates are covered by OPS-003; local backup/restore proof is covered by OPS-004. |
| OPS-003 | PHASE 6 | Add security and dependency scan gates | P1 | OPS-001, OPS-002 | Maybe | Yes | Yes | No | Audit workflow and dependency scan validation | VERIFIED | Added `scripts/ci/security-audit.sh` and CI security-audit job gating frontend/mobile production dependencies on high/critical advisories; added `.github/workflows/security.yml` for PR dependency review plus nightly/manual CodeQL and Trivy filesystem SARIF scans; added `.github/dependabot.yml` for backend Maven, frontend npm, mobile npm, and GitHub Actions updates. Local validation passes: frontend audit has 0 vulnerabilities; mobile audit has known moderate Expo transitive advisories but no high/critical blockers. Backend Maven vulnerability scanning is covered by CodeQL/Trivy/Dependabot in GitHub security workflows; image publish and real deploy security gates remain follow-ups. |
| OPS-004 | PHASE 6 | Add local PostgreSQL backup/restore drill | P1 | OPS-002 | Yes | No | No | No | Backup/restore drill validation | VERIFIED | Files changed: `infra/docker/local/compose.staging.yml`, `infra/env/staging.example.env`, `scripts/ops/backup-local-postgres.sh`, `scripts/ops/restore-local-postgres-drill.sh`, `scripts/ci/validate-ops.sh`, `.gitignore`, `backend/pom.xml`, PostgreSQL-compatible text migrations, and matching entity text mappings. Validation: local staging compose starts PostgreSQL 16 plus backend; backend Flyway V1-V26 migrations apply on PostgreSQL; backup script writes a custom-format `pg_dump` plus manifest under ignored `infra/backup/local`; restore drill restores into `cloudcampus_restore_drill`, verifies 45 public tables, then drops the drill database. Remaining gaps: managed cloud backup scheduling, encrypted offsite retention, formal RTO/RPO, production restore runbook, and CI/scheduled DR drill. |
| DEPLOY-001 | PHASE 6 | Add root local/staging/prod Docker Compose files | P1 | OPS-002 | Yes | Yes | No | No | Compose config validation | VERIFIED | Added `docker-compose.local.yml`, `docker-compose.staging.yml`, and `docker-compose.prod.yml` with PostgreSQL, backend, frontend, localhost-only app ports, healthchecks, and env-driven image names. Compose config validation passes for all three files with safe example env files. |
| DEPLOY-002 | PHASE 6 | Add deployment env templates | P1 | DEPLOY-001 | Yes | Yes | No | No | Secret placeholder validation | VERIFIED | Added `.env.example`, `.env.staging.example`, and `.env.production.example` with safe placeholders, real backend env names including `CLOUDCAMPUS_AUTH_JWT_SECRET`, SMTP placeholders, disabled Super Admin bootstrap, and optional `VITE_API_BASE_URL`. `.gitignore` allows only example env files. |
| DEPLOY-003 | PHASE 6 | Add host Nginx reverse proxy config | P1 | DEPLOY-001 | No | Yes | No | No | Ops validation | VERIFIED | Added `infra/nginx/cloudcampus.conf` with HTTP-to-HTTPS redirect, backend/frontend upstreams, health routing, `/v1` API proxying, secure headers, request size limit, and basic auth/API rate-limit zones. Real domain/cert paths remain environment-specific. |
| DEPLOY-004 | PHASE 6 | Add deployment, rollback, and health-check runbooks | P1 | DEPLOY-001 | Maybe | Maybe | Maybe | No | Documentation review | VERIFIED | Added `docs/deployment/DEPLOYMENT_PLAN.md`, `DEPLOYMENT_README.md`, `HEALTH_CHECK_GUIDE.md`, and `ROLLBACK_GUIDE.md`, documenting EC2 Docker MVP, AWS long-term path, health checks, rollback, staging/prod checklists, and remaining production gaps. |
| DEPLOY-005 | PHASE 6 | Add Docker image build/scan/manual deploy workflow | P1 | OPS-003, DEPLOY-001 | Yes | Yes | No | No | Workflow and ops validation | VERIFIED | Added `.github/workflows/deploy.yml` to build backend/frontend images, Trivy-scan them, optionally push to GHCR, and manually deploy staging/production through GitHub environment secrets and approvals. Hosted target secrets are not configured in repo. |
| DEPLOY-006 | PHASE 6 | Add optional split-hosting API base support | P2 | DEPLOY-002 | No | Yes | No | No | Frontend tests/build | VERIFIED | Added `frontend/src/shared/api/apiBase.ts`, tests, `VITE_API_BASE_URL` typing, and app bootstrap wiring. Same-origin `/v1` remains default; split frontend/backend hosting can set `VITE_API_BASE_URL`. Focused tests, full frontend tests, lint, typecheck, and build pass. |
| DEPLOY-007 | PHASE 6 | Validate local Docker deployment stack | P1 | DEPLOY-001, DEPLOY-006 | Yes | Yes | No | No | Local compose smoke | VERIFIED | Built and started local Docker stack with `.env.example`; PostgreSQL, backend, and frontend became healthy on `127.0.0.1:15432`, `localhost:18080`, and `localhost:18088`. Smoke script passed backend health, readiness, and frontend checks. Frontend healthchecks now use `127.0.0.1` to avoid BusyBox `wget` IPv6 localhost refusal. |
| DEPLOY-008 | PHASE 6 | Add Ubuntu EC2 bootstrap script | P1 | DEPLOY-004 | Maybe | Maybe | No | No | Shell validation | VERIFIED | Added `infra/scripts/ec2-bootstrap-ubuntu.sh` to install Docker, Docker Compose plugin, Nginx, Certbot, UFW, unattended upgrades, app user/directory, backup directory, and Docker log rotation for Ubuntu EC2 hosts. Script syntax and ops validation pass; it was not executed on a real EC2 host in this repo validation. |
| DEPLOY-009 | PHASE 6 | Add EC2 compose deployment helper | P1 | DEPLOY-005, DEPLOY-008 | Yes | Yes | No | No | Dry-run deploy validation | VERIFIED | Added `infra/scripts/ec2-deploy-compose.sh` for staging/production compose deploys, dry-run config validation, production bootstrap blocking, image pulling, optional smoke checks, and service-scoped rollback. GitHub deploy workflow now calls this server-side helper. Staging and production dry-run validation pass with example env files. |
| DOC-001 | PHASE 0 | Keep legacy docs merged into this file only | P2 | ARCH-003 | No | No | No | No | Tracked docs check | VERIFIED | Legacy planning content is merged into this master file; approved deployment runbooks now live under `docs/deployment/*` and are referenced here. This recheck labels historical content and updates overclaimed task statuses. |

## 25TD. AI-001 Verification Notes

Task scope: define the tenant AI entitlement and raw-prompt-safe usage audit foundation. This does not implement live model calls, RAG retrieval, knowledge documents, prompt templates, or user-facing AI workflows.

Files changed:

- `backend/src/main/resources/db/migration/V25__ai_entitlement_audit_foundation.sql`
- `backend/src/main/java/com/cloudcampus/intelligence/ai/*`
- `backend/src/main/java/com/cloudcampus/audit/AuditAction.java`
...
- Added `ai_tenant_entitlements` with tenant-owned enablement, monthly unit budget, enabled feature set, human-approval requirement, retention days, and authenticated updater.
- Added `ai_request_audits` with tenant, school, user, role, feature, scope, request type, prompt SHA-256 hash, prompt length, estimated units/cost, status, denial reason, and timestamps.
- Added Super Admin APIs: `GET /v1/super-admin/ai/tenants/{tenantId}/entitlement` and `PUT /v1/super-admin/ai/tenants/{tenantId}/entitlement`.
- Added authenticated scaffold APIs: `GET /v1/ai/entitlement` and `POST /v1/ai/usage/audit`.
- Server derives tenant, role, user, and active school from the Bearer token/session resolver; request body `tenantId` and `schoolId` are ignored for usage audit and entitlement update scope.
- Raw prompts are not stored in the AI audit table or audit-log metadata; only SHA-256 hash and length are persisted.
- Added typed audit actions: `AI_ENTITLEMENT_UPDATED`, `AI_USAGE_AUDITED`, and `AI_USAGE_DENIED`.

Tests added:

- `AiGovernanceFlowTest` verifies Super Admin can configure tenant AI entitlement without trusting body tenant spoofing.
- Verifies unauthenticated and non-Super Admin entitlement updates are rejected.
- Verifies spoofed tenant headers are blocked by the existing tenant-context spoofing filter.
- Verifies usage audit uses server-derived tenant, school, user, and role, not request body tenant/school values.
- Verifies raw prompt text is absent from persisted request audit metadata and audit logs.
- Verifies disabled tenant AI, disabled feature, and monthly budget overage are denied and audited.
- `AuditCoverageMatrixTest` now includes the AI entitlement and usage mutation controllers.

Validation evidence:

- `cd backend && mvn -q -Dtest=AiGovernanceFlowTest,AuditCoverageMatrixTest test` passed.
- `cd backend && mvn -q test` passed at AI-001 completion; the current full backend suite now reports 133 tests after V27 timetable/document/website school-isolation coverage was added.

Status decision: `AI-001` is VERIFIED for the current backend AI governance foundation. Live model execution provider calls, vector RAG, AI feedback, and AI UI/mobile workflows are intentionally outside this task.

Recommended next implementation task: continue first-version backend/product readiness rather than expanding AI; live model/vector/provider work belongs to a later AI version. `SEC-002`/`SEC-006` are verified for the current scaffold and should remain mandatory regression gates for every future rebuilt school-scoped module.

## 25TE. AI-002 First-Version AI Verification Notes

Task scope: complete only the AI foundation necessary for a first version. This implements entitlement-gated, school-scoped knowledge document retrieval for `SCHOOL_POLICY_QA` without live model calls, vector embeddings, provider secrets, prompt-template management, AI feedback, or frontend/mobile AI UX.

Backend files added or changed:

- `backend/src/main/resources/db/migration/V26__ai_scoped_knowledge_retrieval.sql`
- `backend/src/main/java/com/cloudcampus/intelligence/ai/AiKnowledgeDocument.java`
- `backend/src/main/java/com/cloudcampus/intelligence/ai/AiKnowledgeDocumentRepository.java`
- `backend/src/main/java/com/cloudcampus/intelligence/ai/AiKnowledgeDocumentStatus.java`
- `backend/src/main/java/com/cloudcampus/intelligence/ai/AiKnowledgeDocumentRequest.java`
- `backend/src/main/java/com/cloudcampus/intelligence/ai/AiKnowledgeDocumentResponse.java`
- `backend/src/main/java/com/cloudcampus/intelligence/ai/AiKnowledgeResultResponse.java`
- `backend/src/main/java/com/cloudcampus/intelligence/ai/AiScopedRetrievalRequest.java`
- `backend/src/main/java/com/cloudcampus/intelligence/ai/AiScopedRetrievalResponse.java`
- `backend/src/main/java/com/cloudcampus/intelligence/ai/AiRetrievalAudit.java`
- `backend/src/main/java/com/cloudcampus/intelligence/ai/AiRetrievalAuditRepository.java`
- `backend/src/main/java/com/cloudcampus/intelligence/ai/AiKnowledgeRetrievalService.java`
- `backend/src/main/java/com/cloudcampus/intelligence/ai/SchoolAdminAiKnowledgeController.java`
...
- AI retrieval is enabled only when the authenticated user's tenant has AI enabled and `SCHOOL_POLICY_QA` is present in `enabledFeatures`.
- School Admin knowledge documents are created and listed only for the active school after `SchoolAccessService.requireSchoolAdminAccess`.
- Retrieval ignores client-supplied `schoolId` for access expansion. Scope is derived from authenticated context: active school grant for School Admin/Staff/Finance Staff, active Teacher grant for Teacher, linked child for Parent, and own student profile for Student.
- `SUPER_ADMIN` and `TENANT_ADMIN` cannot use school knowledge retrieval without a school portal scope.
- Knowledge documents have role visibility; `SUPER_ADMIN` and `TENANT_ADMIN` are not accepted as document-visible portal roles.
- Retrieval audit stores query SHA-256, query length, scope, role, result count, and denial reason. Raw user query text is not stored in retrieval audit rows or audit-log metadata.
- Knowledge document creation audit metadata does not store raw knowledge content.

Tests added:

- `AiScopedRetrievalFlowTest` verifies entitlement-gated school retrieval, body `schoolId` spoof resistance, Teacher active-school grant scope, School Admin knowledge creation scope, Parent linked-child retrieval, unlinked-parent denial, Student own-profile retrieval, disabled-entitlement denial, and raw-query/raw-content audit safety.
- `AuditCoverageMatrixTest` now includes AI entitlement, AI usage, AI knowledge document creation, and AI retrieval audit/denial mutation coverage.
- `SchoolScopedControllerGuardCoverageTest` now recognises the guarded School Admin AI knowledge controller in the current scaffold inventory.

Validation evidence:

- `cd backend && mvn -q -Dtest=AiGovernanceFlowTest,AiScopedRetrievalFlowTest,AuditCoverageMatrixTest test` passed.
- `cd backend && mvn -q test` passed.
- Surefire reports 133 tests, 0 failures, 0 errors, 0 skipped.

Remaining AI gaps deferred to the next AI version:

- Live model gateway/provider execution.
- Vector embeddings and production RAG pipeline.
- Prompt-template governance and prompt-injection test corpus beyond current scope-spoofing regression.
- AI response storage/feedback/human review workflow.
- Production provider secrets and cost telemetry.
- Frontend/mobile AI assistant UX.

Status decision: `AI-002` is VERIFIED for first-version backend scoped school knowledge retrieval. It is not a complete production AI copilot; all extra AI integrations are intentionally deferred.

Recommended next implementation task: keep moving on first-version backend/product readiness rather than expanding AI: complete the next non-AI backend gap from the master plan, or add a minimal frontend AI surface only after approval.

## 26. Validation Dashboard

| Validation Area | Command / Method | Last Result | Date | Evidence / Notes |
|---|---|---|---|---|
| Backend Build | `mvn -f backend/pom.xml test` | PASS | 2026-05-27 | Spring Boot 3.5.14 scaffold compiles and tests with Flyway V1-V27, env-configurable H2 dev/test datasource defaults, explicit Flyway PostgreSQL database support, and text mappings compatible with local PostgreSQL staging. |
| Backend Tests | `mvn -f backend/pom.xml test`; `cd backend && mvn test -Dtest=SuperAdminPlatformControlFlowTest` | PASS | 2026-05-28 | Surefire reports 150 tests, 0 failures, 0 errors, 0 skipped after Super Admin platform control coverage. Current coverage includes context/readiness, onboarding, invitation acceptance/email delivery, subscription plan/assignment/invoice foundation, AI entitlement/usage-audit governance, first-version scoped AI knowledge retrieval, tenant/school spoofing, school isolation, role dashboards, teacher marks entry, and Super Admin platform APIs for tenants, schools, revenue, reports, audit logs, platform health, notifications and safe settings. `SuperAdminPlatformControlFlowTest` verifies real platform data, tenant status audit actor attribution, audit metadata redaction, masked notification recipients, unauthenticated/wrong-role denial and spoofed tenant-header rejection. |
...
| Frontend Build | `cd frontend && npm run build` | PASS | 2026-05-28 | React/Vite shell builds successfully with premium public SaaS homepage, shared HTTP client, slide-over universal login/invitation access, auth state provider, current-user/school hydration, protected role panels, premium authenticated app shell, Super Admin API-backed control center pages for dashboard, tenants, schools, subscriptions, revenue, AI usage, reports, audit logs, platform health, notifications and settings, Tenant Admin scaffolds, School Admin operational panels, Teacher marks entry, Parent/Student portal panels, and Finance Staff fee lifecycle scaffold. Vite reports a large chunk warning; code splitting remains a future optimization. |
| Frontend Lint | `cd frontend && npm run lint` | PASS | 2026-05-28 | ESLint passes after Super Admin platform page/API integration and shared API-client usage. |
| Frontend Typecheck | `cd frontend && npm run typecheck` | PASS | 2026-05-28 | TypeScript no-emit check passes with Super Admin platform API types/pages, shared HTTP client, portal resource panels, Teacher marks entry, and existing role scaffolds. |
| Frontend Tests | `cd frontend && npm test -- --run`; `cd frontend && npm test -- --run App.test.tsx` | PASS | 2026-05-28 | 75 Vitest tests pass across 21 files. Coverage includes public homepage/login, auth guards, role alignment, school activation, Super Admin API-backed dashboard/onboarding token forwarding, Tenant Admin scaffolds, School Admin resource panels, Teacher marks entry, Parent/Student portal calls, Finance Staff fee lifecycle, central HTTP client behavior, loading/empty/error states, token forwarding, and the all-role no-pending-navigation regression. |
...
| CI Workflow | `ruby -e "require 'yaml'; YAML.load_file('.github/workflows/ci.yml')"`; `cd backend && mvn -B test`; `cd frontend && npm ci`; `cd frontend && npm test -- --run`; `cd frontend && npm run lint`; `cd frontend && npm run typecheck`; `cd frontend && npm run build`; `cd mobile && npm ci`; `cd mobile && npm run lint`; `cd mobile && npm run typecheck`; `cd mobile && npm test` | PASS | 2026-05-27 | CI-001 plus OPS-001 verified: `.github/workflows/ci.yml` parses as YAML and defines backend, frontend, and mobile jobs using Java 21 and Node 24. Frontend and mobile jobs now run lint gates in addition to tests/typecheck/build. Current local command coverage passes with frontend 57 tests, frontend lint/typecheck/build, mobile lint/typecheck, and 2 mobile tests. Backend and mobile commands were not rerun for FRONT-UX-001/002 because no backend/mobile code or CI step changed. |
| Mobile Typecheck | `cd mobile && npm run typecheck` | PASS | 2026-05-27 | Expo/React Native TypeScript shell passes with backend role codes including `FINANCE_STAFF` and school-switching readiness markers in the shell model. |
| Mobile Tests | `cd mobile && npm test` | PASS | 2026-05-27 | 2 Vitest shell-model tests pass, including backend role coverage, `FINANCE_STAFF`, and switch-ready role coverage. |
| Mobile Lint | `cd mobile && npm run lint` | PASS | 2026-05-27 | OPS-001 verified: ESLint flat config exists for the mobile scaffold, lint dependencies are lockfile-pinned, `npm run lint` passes locally, and CI now runs mobile lint before typecheck/tests. |
...
| Docker Startup | `docker compose -f infra/docker/local/compose.staging.yml up -d --build postgres backend`; `docker inspect --format='{{.State.Health.Status}}' local-backend-1`; `docker compose -f infra/docker/local/compose.staging.yml down` | PASS | 2026-05-27 | Local staging compose starts PostgreSQL 16 and backend with conflict-safe host ports. Backend became healthy after applying Flyway PostgreSQL support and PostgreSQL-compatible text mappings; the stack was stopped after validation. Frontend compose startup was not rerun for OPS-004 because no frontend files changed. |
| API Smoke Tests | `cd backend && mvn -q test` | PARTIAL | 2026-05-27 | MockMvc covers readiness, SUPER_ADMIN-protected tenant onboarding, SUPER_ADMIN subscription plan create/list/update, tenant subscription read/assign, tenant invoice listing, AI entitlement management, AI usage audit, first-version AI knowledge document create/list/search, TENANT_ADMIN school creation with limit checks, TENANT_ADMIN School Admin invite/access grant, TENANT_ADMIN school/admin management, TENANT_ADMIN settings/update/subscription usage, TENANT_ADMIN report summary and school drilldown, invitation acceptance, login, MFA verification, current user, school list, school activation, secure multi-school activation, parent-link creation, parent child list/read, staff/teacher provisioning, academic year create/list/activate, class create/list, section create/list, subject create/list, class-subject create/list, teacher assignment create/list/filter, teacher own-assignment list/filter, student import template/validate/import/list/queue-job/read-job, student login invitation, student self-profile, bulk job create/list/read/cancel, fee demand create/list/read/payment, parent linked-child fee list/payment, student own-fee list, attendance session create/list/read for School Admin and Teacher, homework create/list/read for School Admin and Teacher, parent linked-child homework list, student own-homework list/submission, exam create/list/read/marks/publish for School Admin and Teacher, parent linked-child results, student own results, notice create/list/read/publish for School Admin, teacher notice list, parent linked-child notice list, student own notice list, report export request/list/read/download for School Admin, timetable entry create/list/read, document metadata create/list/read, website page create/list/read/publish, reserved `MAIN` rejection, invitation/login/MFA/onboarding/subscription/AI/tenant-school/tenant-admin-invite/tenant-admin-settings/tenant-report/staff/academic/student/bulk/fee/attendance/homework/exam/notice/report/timetable/document/website authorization negative cases, authenticated onboarding/subscription/AI/tenant-school/tenant-admin-invite/tenant-admin-settings/parent-link/staff/academic/student/bulk/fee/attendance/homework/exam/notice/report/timetable/document/website/auth-session audit persistence, spoofed tenant/school header rejection, and internal queued student import/report processing through service-level worker methods. Live server smoke remains separate from MockMvc coverage. |
| Super Admin Platform Control Tests | `cd backend && mvn test -Dtest=SuperAdminPlatformControlFlowTest`; `cd frontend && npm test -- --run App.test.tsx`; `cd frontend && npm run lint`; `cd frontend && npm run typecheck`; `cd frontend && npm run build` | PASS | 2026-05-28 | SUPER-ADMIN-PORTAL-001 verified for the current scaffold: backend provides SUPER_ADMIN-only APIs for tenants, schools, revenue, reports, audit logs, platform health, notifications and safe settings; unauthenticated and wrong-role callers are denied; spoofed tenant headers are blocked; tenant status mutations are audited with the authenticated actor; audit/notification responses redact secrets. Frontend Super Admin sidebar items render API-backed pages through the shared HTTP client. Remaining partials: inline AI entitlement editing, subscription assignment/edit drawers, durable platform export worker, durable platform settings persistence and payment collection/reconciliation. |
...
| Tenant Admin Settings and Usage Tests | `cd backend && mvn -q -Dtest=SuperAdminSubscriptionFlowTest,TenantAdminSettingsFlowTest,AuditCoverageMatrixTest test`; `cd backend && mvn -q test` | PASS | 2026-05-27 | MUL-007 plus SUB-001 verified: Tenant Admin can read default tenant settings, update same-tenant settings, and view server-derived usage counts for schools, School Admins, teachers, staff, and students. Usage returns `SCAFFOLD` when no subscription is assigned and the assigned plan code after Super Admin assignment. Body tenant spoofing does not create/update another tenant's settings; spoofed tenant headers, unauthenticated callers, and non-Tenant Admin callers are rejected. Audit rows contain authenticated Tenant Admin actor and do not contain raw tenant settings emails or spoofed tenant ids. Frontend Tenant Admin settings/usage scaffold was not changed for SUB-001. |
| Subscription Plan and Assignment Tests | `cd backend && mvn -q -Dtest=SuperAdminSubscriptionFlowTest,AuditCoverageMatrixTest,TenantAdminSettingsFlowTest,TenantAdminSchoolManagementFlowTest test`; `cd backend && mvn -q test` | PASS | 2026-05-27 | SUB-001 verified: Flyway V23 creates `subscription_plans`, `tenant_subscriptions`, and `tenant_invoices`; Super Admin can create/list/update plans, assign a plan to a tenant, optionally issue an invoice, and list invoices. Assigned plans update `tenant_school_limits`; Tenant Admin usage returns the assigned plan; Tenant Admin school creation enforces the plan's school limit. Unauthenticated callers, `SCHOOL_ADMIN`, `TENANT_ADMIN`, spoofed role/body tenant values, spoofed tenant headers, unsafe plan assignment, and plan downgrades below current school usage are blocked. Audit rows contain the authenticated Super Admin actor and omit spoofed tenant ids. |
...
| Fee Lifecycle Tests | `cd backend && mvn -q -Dtest=FeeLifecycleFlowTest,AuditCoverageMatrixTest,SchoolScopedControllerGuardCoverageTest test`; `cd backend && mvn -q test`; `cd frontend && npm test -- --run`; `cd frontend && npm run typecheck`; `cd frontend && npm run build` | PASS | 2026-05-27 | FEE-001 plus FEE-002 verified: Flyway V14 adds `fee_demands` and `fee_payments`; School Admin can create/list/read demands and record payments; Finance Staff can create/list/read demands and record payments through `/v1/finance/...` for assigned schools only; parent can view/pay only linked child's demand; student can view own demands; payments update partial/paid state and issue receipts; School Admin A and Finance Staff A cannot create/read/pay School B demand; parent unlinked/mismatched demand access and overpayment are rejected; audit rows avoid raw gateway references; frontend fee scaffold forwards Bearer auth. |
| Finance Role Tests | `cd backend && mvn -q -Dtest=FeeLifecycleFlowTest,StaffProvisioningFlowTest,AuthSessionFlowTest,TenantOnboardingFlowTest test`; `cd frontend && npm test -- --run App authApi StaffProvisioningPage FeeLifecyclePage`; `cd mobile && npm test -- --run`; `cd backend && mvn -q test`; `cd frontend && npm test -- --run`; `cd frontend && npm run typecheck`; `cd frontend && npm run build`; `cd mobile && npm run typecheck`; `cd mobile && npm test` | PASS | 2026-05-27 | FEE-002 verified: `FINANCE_STAFF` is a backend/web/mobile role; School Admin can provision Finance Staff through the staff invite flow; privileged MFA applies; Finance Staff can use finance fee demand/payment routes for assigned school access; Finance Staff cannot access academic admin routes or another school's fee demand; web role guards show the Finance Staff area and hide admin-only surfaces; mobile shell model includes the role. |
...
| Academic Selector UX Tests | `cd frontend && npm test -- --run`; `cd frontend && npm run lint`; `cd frontend && npm run typecheck`; `cd frontend && npm run build` | PASS | 2026-05-28 | UX-ACA-FORM-001 verified: School Admin academic setup loads academic years/classes/sections from real APIs, hides raw academic-year/class IDs, creates classes and sections from selectors, lists existing records, and handles loading/empty/error/success states. Academic assignments load subjects, class-subjects, and teachers, use searchable teacher/class/subject selectors, hide raw IDs, and send the expected API payloads. |
...
| Ops/Staging Scaffold | `sh scripts/ci/validate-ops.sh`; `docker compose -f infra/docker/local/compose.staging.yml config`; `sh scripts/ops/smoke-staging.sh`; `CLOUDCAMPUS_SMOKE_EMAIL=superadmin@cloudcampus.dev CLOUDCAMPUS_SMOKE_PASSWORD='SuperAdmin123!' sh scripts/ops/smoke-staging.sh`; `docker build -f backend/Dockerfile -t cloudcampus-backend:ops002 .`; `docker build -f frontend/Dockerfile -t cloudcampus-frontend:ops002 .` | PASS | 2026-05-27 | OPS-002 verified for repo-level staging readiness: required ops files exist, deploy files reject committed dev bootstrap password, staging compose config parses, local health/readiness/frontend smoke checks pass, optional login smoke returns a valid auth/MFA response through the frontend proxy, backend/frontend container images build, and frontend image build keeps the existing Vite large chunk warning only. Hosted staging, managed secrets, image publish, TLS, and deployed alerts remain future hardening. Local backup/restore proof is covered by OPS-004. |
| Deployment Structure | `sh scripts/ci/validate-ops.sh`; `docker compose --env-file .env.example -f docker-compose.local.yml config`; `docker compose --env-file .env.staging.example -f docker-compose.staging.yml config`; `docker compose --env-file .env.production.example -f docker-compose.prod.yml config`; `cd frontend && npm test -- --run`; `cd frontend && npm run lint`; `cd frontend && npm run typecheck`; `cd frontend && npm run build`; `docker compose --env-file .env.example -f docker-compose.local.yml up -d --build`; `BACKEND_URL=http://localhost:18080 FRONTEND_URL=http://localhost:18088 sh scripts/ops/smoke-staging.sh`; `DRY_RUN=true DEPLOY_ENV=staging APP_DIR="$PWD" ENV_FILE=.env.staging.example sh infra/scripts/ec2-deploy-compose.sh`; `DRY_RUN=true DEPLOY_ENV=production APP_DIR="$PWD" ENV_FILE=.env.production.example sh infra/scripts/ec2-deploy-compose.sh` | PASS | 2026-05-27 | DEPLOY-001 through DEPLOY-009 verified for repo-level deployment readiness: root compose files render, env templates are safe placeholders, Nginx reverse proxy template exists, deployment/health/rollback runbooks exist, deploy workflow builds/scans/pushes images and calls the EC2 helper, optional `VITE_API_BASE_URL` works, local Docker stack is healthy, EC2 bootstrap script syntax passes, and staging/production deploy helper dry-runs pass. Remaining gaps are real EC2/RDS/domain/TLS/secrets configuration and hosted smoke validation. |
| School-Scoped Controller Guard Coverage | `rg -n "@RestController|@Controller|@RequestMapping|@GetMapping|@PostMapping|@PutMapping|@PatchMapping|@DeleteMapping" backend/src/main/java`; `cd backend && mvn -q -Dtest=SchoolScopedMissingModulesFlowTest,SchoolScopedControllerGuardCoverageTest,AuditCoverageMatrixTest test`; `cd backend && mvn -q test` | PASS | 2026-05-27 | Current rebuilt guarded school-scoped controllers include PAR-001 parent child read routes, STAFF-001 staff provisioning route, ACA-001 academic year/class/section routes, ACA-002 subject/class-subject/teacher assignment routes, STU-001/STU-002 student import/list/import-job routes, STU-003 student login/self-profile routes, BULK-001 bulk job routes, FEE-001 fee demand/payment routes, ATT-001 attendance routes, HOME-001 homework routes, EXAM-001 exam/result routes, NOTE-001 notice routes, REP-001 report export routes, timetable routes, document routes, website page routes, and AI knowledge routes. `SchoolScopedControllerGuardCoverageTest` fails future school-scoped controllers that lack an approved backend guard marker. |
...
| Invitation Email Delivery Tests | `cd backend && mvn -q -Dtest=InvitationEmailDeliveryFlowTest,TransactionalOutboxFlowTest test`; `cd backend && mvn -q test` | PASS | 2026-05-27 | NOTIF-001 verified: Flyway V22 creates `notification_deliveries`; `spring-boot-starter-mail` and env-backed `cloudcampus.notifications.email.*` config are present; onboarding, Tenant Admin School Admin invite/resend, parent invite, staff/teacher invite, and student-login invite paths create delivery rows and token-safe `InvitationEmailDeliveryRequested` outbox events. SMTP mode sends the invitation acceptance URL through `JavaMailSender`; default local mode logs only masked recipients and marks deliveries `LOGGED`; tests verify delivery status/provider persistence and prove raw invitation tokens, token hashes, and acceptance URLs are not stored in delivery rows or outbox payloads. |
...
| Security Scan | `ruby -e "require 'yaml'; YAML.load_file('.github/workflows/ci.yml'); YAML.load_file('.github/workflows/security.yml'); YAML.load_file('.github/dependabot.yml')"`; `sh scripts/ci/security-audit.sh` | PASS | 2026-05-27 | OPS-003 verified: CI includes a high/critical production npm audit gate; `.github/workflows/security.yml` defines PR dependency review, CodeQL, and Trivy filesystem SARIF scans; Dependabot covers backend Maven, frontend npm, mobile npm, and GitHub Actions. Workflow YAML parses locally. CodeQL/Trivy run in GitHub, not locally. |
| Dependency Scan | `sh scripts/ci/security-audit.sh`; `cd frontend && npm audit --omit=dev --audit-level=high`; `cd mobile && npm audit --omit=dev --audit-level=high` | PASS/PARTIAL | 2026-05-27 | Frontend production audit: 0 vulnerabilities. Mobile Expo 56 production audit still reports 10 moderate transitive advisories, 0 high, 0 critical; npm suggests a breaking Expo downgrade, so the first-version gate blocks high/critical only while keeping moderate Expo advisories documented for future dependency upgrades. |
| Backup/Restore Test | `docker compose -f infra/docker/local/compose.staging.yml up -d --build postgres backend`; `sh scripts/ops/backup-local-postgres.sh`; `sh scripts/ops/restore-local-postgres-drill.sh`; `mvn -f backend/pom.xml test`; `sh scripts/ci/validate-ops.sh`; `docker compose -f infra/docker/local/compose.staging.yml down` | PASS/PARTIAL | 2026-05-27 | OPS-004 verified local backup/restore only: backend ran healthy against PostgreSQL 16 with Flyway V1-V26 at that checkpoint, backup script created ignored custom-format dump `infra/backup/local/cloudcampus-cloudcampus-20260527T064121Z.dump` plus manifest, restore drill restored into `cloudcampus_restore_drill`, verified 45 public tables, and dropped the drill DB. Current backend tests pass with Flyway V1-V27 and 133 tests. Remaining production gaps: managed cloud backups, encrypted offsite retention, scheduled restore drill, RTO/RPO, and production runbook. |
