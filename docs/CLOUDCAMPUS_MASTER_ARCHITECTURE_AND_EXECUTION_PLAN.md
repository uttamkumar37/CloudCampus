...
| Infrastructure | Docker, local staging compose, Nginx, env contract, monitoring alert rules, backup/load-test placeholders | Backend/frontend Dockerfiles, local staging compose, frontend Nginx proxy config, staging env example, smoke script, and Prometheus alert-rule scaffold exist | Repo-level staging scaffold exists and builds locally. No hosted staging environment, managed production secret store, image registry publish, TLS/domain, backup/restore workflow, or deployed Prometheus/Grafana/Alertmanager stack exists yet. | Required before pilot: choose hosting target, wire managed PostgreSQL/secrets/TLS/registry/backup/alerts, and prove restore. |
| CI/CD | `.github/workflows/ci.yml`, `.github/workflows/security.yml`, `.github/dependabot.yml`, plus `.github/workflows/.gitkeep` | GitHub Actions workflow present for backend, frontend, mobile, ops scaffold, security audit, and nightly/manual security scans | Current CI runs backend Maven tests, frontend `npm ci`/tests/lint/typecheck/build, mobile `npm ci`/lint/typecheck/tests, ops scaffold validation plus staging compose config, and high/critical npm production audit gates. Security workflow adds PR dependency review, CodeQL, and Trivy filesystem SARIF scanning on PR/nightly/manual. Dependabot is configured for backend Maven, frontend npm, mobile npm, and GitHub Actions. No image publish, OpenAPI publish, DR drill, or hosted staging deploy exists yet. | Required before pilot: add image publish, real staging deploy, OpenAPI publish, backup/restore workflows, and tune scanner false-positive policy as dependencies mature. |
...
Status decision: `OPS-002` is VERIFIED for the current repository-level staging/ops scaffold. Remaining gaps before pilot are a real hosted staging target, managed secret store, production SMTP secret injection, image registry publish, TLS/domain setup, managed PostgreSQL/object storage, Prometheus/Grafana/Alertmanager deployment, backup/restore proof, and DR workflow. Dependency/security scan gates are covered by OPS-003.
