...
| CI/CD | `.github/workflows/ci.yml` plus `.github/workflows/.gitkeep` | GitHub Actions workflow present for backend, frontend, and mobile scaffold gates | Current CI runs backend Maven tests, frontend `npm ci`/tests/lint/typecheck/build, and mobile `npm ci`/lint/typecheck/tests on push, pull request, and manual dispatch. No deploy, image publish, security nightly, OpenAPI publish, DR drill, or dependency/security scan job exists yet. | Required before pilot: add security/deploy gates after manifests and policies exist. |
...
Recommended next implementation task: `PAR-002` parent leave request flow if continuing ERP feature depth, or `OPS-002` staging deploy/secrets/alerts if hardening toward pilot operations. `SEC-002` remains the standing security gate to strengthen alongside each rebuilt school-scoped module.
...
| CI Workflow | `ruby -e "require 'yaml'; YAML.load_file('.github/workflows/ci.yml')"`; `cd backend && mvn -B test`; `cd frontend && npm ci`; `cd frontend && npm test -- --run`; `cd frontend && npm run lint`; `cd frontend && npm run typecheck`; `cd frontend && npm run build`; `cd mobile && npm ci`; `cd mobile && npm run lint`; `cd mobile && npm run typecheck`; `cd mobile && npm test` | PASS | 2026-05-27 | CI-001 plus OPS-001 verified: `.github/workflows/ci.yml` parses as YAML and defines backend, frontend, and mobile jobs using Java 21 and Node 24. Frontend and mobile jobs now run lint gates in addition to tests/typecheck/build. Current local command coverage passes with frontend 53 tests, frontend lint/typecheck/build, mobile lint/typecheck, and 2 mobile tests. Backend command was not rerun for OPS-001 because no backend code or CI backend step changed. |
