...
| Overall readiness score | **76 / 100** (see §19 scorecard). |
| Strongest areas | Critical-mutation audit hooks now exist for fee/payment, student lifecycle, marks/results, notices, leave approvals, settings, academic years, custom domains, parent links, and AI Copilot; irreversible actions now require operator reason capture and store that reason in audit metadata; audit-log viewer endpoints and UI now exist for School Admin and Super Admin; per-tenant restore drill now validates one tenant from an encrypted dump with zero cross-tenant scoped rows; public Privacy Policy, Terms, and DPA pages now exist and are linked; student/parent/teacher self-profile endpoints now feed portal layouts; high-cost/high-abuse endpoints now use the shared API rate limiter; mutating controller bodies are now covered by `@Valid` except the raw Razorpay webhook signature body; CVE/dependency scans now run as a PR-triggered release gate; bulk student promotion now has backend `dryRun=true` protection plus UI preview; payment HMAC + webhook idempotency are real; parent fee payment now exists in the parent portal; first-login forced password-change and prod bootstrap secret enforcement now exist; demo tenant reset hygiene and demo-only labeling now exist; comprehensive observability stack (Prometheus / Grafana / Loki / Tempo); backend full suite passes after P1-04; frontend strict TS/build pass; clean role separation per portal at the URL prefix level. |
| Biggest blockers | (1) Operational pilot checklist still needs formal sign-off before real schools are onboarded. (2) Parent-payment-in-prod hardening, MFA, commercial evidence, customer data export, and external pen-test still block paid sale. (3) Load testing, quotas, and enterprise operations still block scale claims. |
...
**Updated through P1-05 on 2026-05-26.** Counts derived from spot-checking the actual code; see §3 for the per-finding verification matrix.
...
| **Level C — First Paid Customer Ready** | **12 tracked tasks + mandatory non-task evidence** | Parent-payment-in-prod hardening + MFA + **external penetration test (P2-10)** + data export + non-task commercial evidence (pricing, support process, payment reconciliation, security/trust page, completed pilot validation) | FAIL — both tracked tasks AND mandatory evidence required; **pen-test mandatory** | No |
| **Level D — Revenue Expansion Ready** | **20** | All of C + report-card PDF + parent-teacher chat + pilot validation | FAIL | No |
| **Level E — Scale / Enterprise Ready** | **29** | All of D + load test + multi-school proof at scale | FAIL | No |
...
| Already completed in current code (verified by grep + spot read) | 27 |
...
| Remaining before **First Paid Customer Ready (Level C)** | **12** |
| Remaining before Revenue Expansion Ready (Level D) | 20 |
| Remaining before Scale / Enterprise Ready (Level E) | 29 |
...
| Per-tenant restore drill now exists and passes locally. | P1-10 added `DRILL_TENANT_ID` support, tenant-only scratch schema validation, row-count checks for every `tenant_id` table, and a GitHub workflow seed with a target tenant plus bystander tenant. Local containerized drill validated 71 tenant-scoped tables with zero cross-tenant scoped rows. |
| Privacy Policy, Terms, and DPA now exist on the public website. | P1-11 added `/privacy`, `/terms`, `/dpa`, `/legal/privacy`, `/legal/terms`, and `/legal/dpa`, footer links, and static markdown artifacts under `frontend/public/legal/`. Frontend build and route smoke pass. |

---

## 1.2 Simple Founder Answer

> Plain answers a non-engineer can act on. Each number is updated through P1-05 on 2026-05-26.
...
**12 tracked engineering/product/security tasks remain PLUS mandatory non-task commercial/operational evidence.** The remaining tracked-task calculation is **0 pilot tasks + 5 remaining Phase-1 items not already in pilot + 7 Phase-2 paid-readiness items = 12**. The 7 Phase-2 items required for Level C are exactly:
...
> Last updated: 2026-05-26 after P1-05 landed. Tracked via the §17 Top-25 queue order and the §16 phase tables. All counts re-derived from the actual code state at HEAD.
...
| **23** | **P1-10** | Per-tenant restore drill from encrypted backup with tenant-only scratch schema validation | 2026-05-25 | `sh -n infra/pgbackup/drill.sh`; `bash infra/pgbackup/drill.sh --help`; local containerized drill PASS validating 71 tenant-scoped tables with zero cross-tenant scoped rows |
| **24** | **P1-11** | Privacy Policy, Terms, and DPA public pages plus static legal artifacts | 2026-05-25 | Frontend build passes; `/privacy`, `/terms`, and `/dpa` return HTTP 200; static `/legal/*.md` artifacts return content |
| **25** | **P1-02** | Teacher homework and assignment creation from Teacher portal | 2026-05-26 | `MultiSchoolMultiTenantIT#teacherCreatesHomeworkAndAssignmentsInsideOwnSchool` passes; frontend build passes |
| **26** | **P1-03** | Teacher class/section/subject ownership enforcement | 2026-05-26 | Cross-teacher ownership assertions in `MultiSchoolMultiTenantIT#teacherCreatesHomeworkAndAssignmentsInsideOwnSchool` pass |
| **27** | **P1-05** | Async report export jobs with polling and CSV download | 2026-05-26 | `MultiSchoolMultiTenantIT#reportExportJobCompletesAsynchronously` passes; frontend build passes |
...
| Level C — First Paid Customer Ready | **39 tracked + non-task evidence** | **27** | **12 tracked + all non-task evidence** | P2-10 (external) |
| Level D — Revenue Expansion Ready | 47 | 27 | 20 | P2-10 (external) |
| Level E — Scale / Enterprise Ready | 56 | 27 | 29 | P2-10 (external) |
...
| Phase 1 | 14 | 9 | 5 | P1-01, P1-02, P1-03, P1-04, P1-05, P1-09, P1-10, P1-11, and P1-13 complete |
...
| `MultiSchoolMultiTenantIT` async report export job | 1 | PASS (`reportExportJobCompletesAsynchronously` on 2026-05-26) |
| `MultiSchoolMultiTenantIT` alone | 12 | PASS (TI-01..TI-10 + portal `/me` snapshots + audit-log viewer scoping, last full focused run before P1-05) |
...
| Frontend `npm run build` | — | PASS on 2026-05-26 |
...
- **Modules with a real backend + frontend pair:**
  ...
  - Reports (attendance / fees / performance + async CSV export jobs with polling and download).
...
- **Student, parent, and teacher self-profile endpoints now exist (P1-01).** `StudentLayout`, `ParentLayout`, and `TeacherLayout` now query real current-user identity instead of showing only auth-store role labels.
...
- **Reports CSV export now has async job endpoints** (P1-05). Legacy synchronous `/export` routes remain for compatibility; the frontend uses `POST .../export-jobs`, polls `/jobs/{id}`, then downloads `/jobs/{id}/download`.
...
| **Teacher** | Dashboard, timetable, attendance + QR, self-profile shell identity, homework list, assignment grading, lesson plans, online classes, video upload, leave, notices | Create homework/assignments from Teacher portal; marks-entry permission; class-performance reports; exam duties roster; leave-balance | `/v1/teacher/**` not in SecurityConfig matcher list — relies solely on method-level `@PreAuthorize`; no class/section/subject assignment ownership check; assignment grading has no `maxMarks` clamp; attendance edit window not enforced; no audit log; video initiate is rate-limited after P0-16 but QR session generation still needs review; notice endpoint uses `/v1/mobile/notices` (any auth role) | **Works for read-heavy daily use; not commercially complete** | 74 |
| **Student** | Self-profile shell identity, Profile-360 (self), homework + submit, assignments + submit, timetable, attendance + QR self-mark, results, fees, Razorpay payment, notices | Documents read-only, report-card PDF, in-app notifications, leave application, achievements export | P0-02/P0-09/P0-13/P0-17/P1-01 now cover QR role gating, self-profile redaction, multi-school notices, matrix regression, and current-user portal identity. Remaining risks: no audit log on submits/payments and broader production hardening still needed | **Smallest surface; mostly correct on ownership; security gaps blocking** | 76 |
| **Parent** | Parent self-profile shell identity, linked children list, child attendance / homework / results / timetable / fees, notices, Razorpay test-mode fee payment | Child profile/documents, leave application for child, in-app notifications, notice acknowledgement, parent-teacher messaging, upcoming exams | P0-01/P0-13 fixed the `MAIN` school resolver issues; P0-08 fixed draft result leakage; P0-14 added parent payment; P0-17 locks cross-owner and multi-school parent paths; P1-01 adds current-user identity. Remaining risks: profile/consent/chat gaps and production payment hardening | **Readable portal plus demo payment flow; still needs pilot hardening** | 72 |
...
| No `/v1/teacher/me`, `/v1/student/me`, `/v1/parent/me` | Teacher + Student + Parent | ✅ fixed in P1-01: role-scoped endpoints exist and layouts query them | UX blocker closed for pilot demos | **Closed** |
...
| Reports `/export` is synchronous | School Admin | ✅ fixed in P1-05: async export-job endpoints exist for attendance, fees, and performance; frontend creates, polls, and downloads jobs. Legacy sync routes remain for compatibility. | Large schools no longer need to hold a single export request open in the UI | **Closed** |
...
| Privacy policy + terms + DPA | Present as pilot-ready public pages; legal review still required before paid sale | First paid customer (legal requirement) |
...
**Before collecting money:** add BL-07, BL-18, BL-19, complete the remaining Level-C tracked tasks, get legal review/sign-off for Privacy Policy / Terms / DPA, and have a public security disclosure page.
...
| Backup / restore | DR drill monthly + per-tenant restore validation | Local drill restored encrypted dump and validated one tenant with zero cross-tenant scoped rows; monthly workflow should now be watched for first scheduled green run | n/a | Keep monthly evidence and add customer-facing export endpoint | Medium |
...
| TD-15 | Missing feature | `/v1/student/me`, `/v1/parent/me`, `/v1/teacher/me` | P1-01 added role-scoped endpoints and layout integration | UX gap closed for portal shell identity | Keep endpoint regression coverage | Closed |
...
| Phase 1 | Level B (complete); start of Level C | 14 | 9 | 5 |
...
| P1-05 | Async / streaming reports export | Performance | `ReportController.export*` | Returns job; `/jobs/{id}` polls | Manual + tests | Reliability: high | **[x] Done — 2026-05-26** (`POST .../export-jobs` returns `202 Accepted`; `/jobs/{jobId}` polls status; `/jobs/{jobId}/download` returns CSV; frontend export buttons use create/poll/download; focused integration test and frontend build pass) |
...
| P1-10 | Per-tenant restore drill | DR | `infra/pgbackup/drill.sh` + workflow | Workflow restores ONE tenant | Workflow log | Reliability: critical | **[x] Done — 2026-05-25** (`DRILL_TENANT_ID` selects the tenant; drill restores encrypted dump into scratch DB, builds `tenant_restore_scope`, validates every `tenant_id` table has matching target counts and zero cross-tenant scoped rows; local containerized drill validated 71 tenant-scoped tables) |
| P1-11 | Privacy policy + Terms + DPA published on public website | Legal | `frontend/public/legal/*` | Pages exist | Manual | Sale: critical | **[x] Done — 2026-05-25** (`/privacy`, `/terms`, `/dpa`, `/legal/*` routes, footer links, and static markdown artifacts added; frontend build and route smoke pass) |
...
> Re-counted on 2026-05-26 after P1-05. Per-task verification in §3 + §16.
...
- **Important boundary:** demos are ready for demo-only data. Real pilot data now waits on the operational Level-B checklist, not additional Level-B roadmap code.

### Before Level B — Controlled Pilot Ready

- **Required remaining roadmap tasks: 0** (pilot-critical Phase-1 roadmap work is complete; total Phase-0 = 18, total pilot-Phase-1 = 6).
- **Exact task IDs:** none.
- **What real data becomes safe after operational sign-off:** Real students, real parents, real fees (Razorpay test mode for early pilots), real attendance, real notices — for **1–3 carefully chosen single-campus schools** with documented support SLAs. **Not** multi-school. **Not** chain operators. **Not** schools with strict regulatory regimes.
...
- **Required remaining tracked tasks: 12.**
- **Math:** 0 remaining Pilot Ready tasks + **5 remaining Phase-1 tasks not already in Pilot** + **7 required Phase-2 paid-readiness tasks (including P2-10 external pen-test)** = **12**.
- **5 remaining Phase-1 tasks (not in Pilot):** P1-06 (Pagination across endpoints), P1-07 (Parent payment hardening + UI cache invalidation in production), P1-08 (Marks bounds + attendance edit window), P1-12 (React Query invalidation after mutating actions), P1-14 (Bulk staff import).
...
- **Required remaining tasks: 29** (all remaining roadmap tasks through Phase 4).
...
- [x] Per-tenant restore drill PASS.
- [ ] Pilot agreement template ready.
- [x] Privacy policy + Terms + DPA published.
...
**Current status:** **ROADMAP PASS; OPERATIONAL CHECKLIST PENDING** — all 18 Phase-0 tasks and all 6 pilot-critical Phase-1 roadmap tasks are complete, but CI evidence, pilot agreement, support/runbook, and security scan evidence still need sign-off before onboarding real schools.
...
|---|---:|---|---:|
| Product completeness | 69 | All five role portals functional; parent demo payment exists; self-profile shell identity exists for student/parent/teacher; audit viewer exists; still missing teacher homework creation and broader workflow polish | 80 |
| Security | 77 | Strong auth + HMAC + tenant filter + high-cost endpoint rate limits; mutating request bodies now validate; OWASP + Trivy now run as a PR-triggered release gate; irreversible action reasons are captured; remaining gaps are MFA and external pen-test | 85 |
...
| Reliability / DevOps | 76 | Backups + DR drill + per-tenant restore proof + CI + observability are real; CVE/dependency release gate now blocks HIGH/CRITICAL findings; rollback runbook is still missing | 85 |
| Testing | 76 | 248 backend tests pass; cross-tenant / role-matrix / payment / KB-isolation / parent draft-result exclusion / student self-profile redaction / portal self-profile snapshots / audit-log viewer scoping / forced password-change / prod bootstrap / student-promotion dry-run / demo reset hygiene / rate-limit / validation coverage / required-reason / public HTTP 400 / multi-school matrix tests exist; per-tenant restore drill passes locally; per-mutation audit and broad frontend RTL gaps remain | 85 |
| Sales / demo readiness | 64 | Demo tenant + role logins ready; public legal pages now exist; still missing pricing sign-off, pilot agreement, sales collateral, public trust page, and screenshots | 80 |
| Future feature readiness | 70 | Modular architecture; clean feature-flag system; subscription scaffolding | 70 |

- **Overall current score: 76 / 100.**
- **Recommended commercial status:** **Controlled Pilot after the operational checklist passes.** Not "Paid Sale Ready".
- **Exact reason:** Level-B roadmap code is complete, but controlled pilots still need operational evidence: CI green, pilot agreement, incident/runbook readiness, support channel, and current security scan sign-off.
...
- Legal-reviewed Privacy Policy + Terms + DPA.
...
> Last updated 2026-05-26 after P1-05 shipped. See §1.3 for the full progress tracker.

- **Tasks completed so far:** **27 of 56** — P0-01, P0-02, P0-03, P0-04, P0-05, P0-06, P0-07, P0-08, P0-09, P0-10, P0-11, P0-12, P0-13, P0-14, P0-15, P0-16, P0-17, P0-18, P1-01, P1-02, P1-03, P1-04, P1-05, P1-09, P1-10, P1-11, P1-13.
- **Current total remaining roadmap tasks:** **29** (was 56).
- **Remaining Customer Demo Ready (Level A) tasks:** **0** — was 8.
- **Remaining Controlled Pilot Ready (Level B) tasks:** **0** — was 24.
- **Remaining First Paid Customer Ready (Level C) tasks:** **12 tracked + all non-task evidence** — was 39 tracked; includes **P2-10 external penetration test**.
...
- **Files:** external vendor report plus remediation commits for any findings.
...
**P1-06:** Pagination on high-volume list endpoints.

### Validation command that proves P1-05 is completed

```
git diff --check && mvn -f backend/pom.xml -Dtest=MultiSchoolMultiTenantIT#reportExportJobCompletesAsynchronously test && npm --prefix frontend run build
```

Route smoke from 2026-05-25:

```
curl -I http://127.0.0.1:5173/privacy
curl -I http://127.0.0.1:5173/terms
curl -I http://127.0.0.1:5173/dpa
```

Result: all three routes returned HTTP 200; static markdown files under `/legal/` also returned content.

### Warning

**Do not implement multiple unrelated tasks together.** Each roadmap task has its own verification test. Bundling them makes regression diagnosis painful and review slower. Tackle them in the **§17 Top-25 order** strictly. After Task 24 lands and the tests pass, Task 25 (P2-10) is external pen-test evidence, not an in-repo code-only task.
...
- P1-10 per-tenant restore drill: **PASS** — `sh -n infra/pgbackup/drill.sh`, `bash infra/pgbackup/drill.sh --help`, and local containerized drill PASS validating 71 tenant-scoped tables with zero cross-tenant scoped rows (2026-05-25, exit 0).
- P1-11 public legal pages: **PASS** — frontend build passes; `/privacy`, `/terms`, `/dpa` return HTTP 200; static `/legal/privacy-policy.md`, `/legal/terms-of-service.md`, and `/legal/data-processing-addendum.md` return content (2026-05-25, exit 0).
- P1-05 async report export jobs: **PASS** — `MultiSchoolMultiTenantIT#reportExportJobCompletesAsynchronously` creates a job, polls completion, and downloads CSV; frontend build passes (2026-05-26, exit 0).
...
- API smoke against running local backend: **PASS** — `/actuator/health` returned `UP`; frontend dev server returned HTTP 200 on 2026-05-25.
